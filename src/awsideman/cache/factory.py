...
class BackendFactory:
    ...
    def _create_file_backend(config: AdvancedCacheConfig) -> FileBackend:
        ...
        try:
            ...
            raise CacheBackendError(
                f"Failed to create file backend: {e}",
                backend_type="file",
                original_error=e
            )
    ...
    def _create_dynamodb_backend(config: AdvancedCacheConfig) -> CacheBackend:
        ...
        try:
            ...
            logger.debug(f"Creating DynamoDB backend with table: {config.dynamodb_table_name}, "
            ...
            raise CacheBackendError(
                f"Failed to create DynamoDB backend: {e}",
                backend_type="dynamodb",
                original_error=e
            )
    ...
    def _create_hybrid_backend(config: AdvancedCacheConfig) -> CacheBackend:
        ...
        try:
            ...
            raise CacheBackendError(
                f"Failed to create hybrid backend: {e}",
                backend_type="hybrid",
                original_error=e
            )
    ...
    def get_backend_info(backend_type: str) -> dict:
        ...
        info = backend_info.get(backend_type.lower(), {
