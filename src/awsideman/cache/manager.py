...
from ..utils.models import CacheEntry, CacheConfig
...
class CacheManager:
    ...
    def _load_and_validate_configuration(self, config: Optional[Union[CacheConfig, AdvancedCacheConfig]]) -> Union[CacheConfig, AdvancedCacheConfig]:
        ...
        try:
            ...
            if validation_errors:
                ...
                has_critical_errors = any(field in critical_errors for field in validation_errors.keys())
                if has_critical_errors:
                    logger.warning("Critical configuration errors detected, falling back to basic configuration")
            ...
            if self._is_advanced_config_meaningful(advanced_config):
                ...
                logger.debug("Advanced configuration contains only defaults, using basic configuration for compatibility")
                return self._load_basic_cache_config()
    ...
    def _is_advanced_config_meaningful(self, config: AdvancedCacheConfig) -> bool:
        ...
        if (config.dynamodb_table_name != "awsideman-cache" or
            config.dynamodb_region is not None or
            config.dynamodb_profile is not None):
            return True
    ...
    def _initialize_path_manager(self, base_cache_dir: Optional[str]) -> None:
        ...
        if isinstance(self.config, CacheConfig) and not isinstance(self.config, AdvancedCacheConfig):
    ...
    def _initialize_advanced_features(self, backend: Optional[CacheBackend], 
                                    encryption_provider, base_cache_dir: Optional[str]) -> None:
        ...
        try:
            ...
            logger.info(f"Successfully initialized cache with backend: {type(self.backend).__name__}, "
    ...
    def _initialize_backend_and_encryption(self, backend, encryption_provider, base_cache_dir):
        ...
        if isinstance(self.config, AdvancedCacheConfig):
            logger.debug(f"Initializing advanced cache with backend: {self.config.backend_type}, "
            ...
            logger.debug("Using basic configuration - backend and encryption handled by legacy methods")
            self.backend = None
            self.encryption_provider = None
    
    def _initialize_backend(self, backend: Optional[CacheBackend], base_cache_dir: Optional[str]) -> None:
        ...
        if not BackendFactory.validate_backend_availability(self.config.backend_type):
            ...
            if "file" in available_backends:
                ...
                try:
                    ...
                    logger.info(f"Successfully fell back from {original_backend_type} to file backend")
                    return
                except Exception as e:
                ...
                raise CacheBackendError(f"No available backends found")
            ...
            try:
                if hasattr(self.backend, 'health_check') and callable(self.backend.health_check):
                    if not self.backend.health_check():
                        logger.warning(f"Backend health check failed for {type(self.backend).__name__}")
                    else:
                        logger.debug(f"Backend health check passed for {type(self.backend).__name__}")
    ...
    def _initialize_encryption(self, encryption_provider) -> None:
        ...
        if encryption_provider is not None:
            logger.debug(f"Using provided encryption provider: {type(encryption_provider).__name__}")
        ...
        try:
            ...
            try:
                ...
                raise EncryptionError(f"Failed to initialize any encryption provider: {fallback_error}")
        except Exception as e:
            ...
            try:
                ...
                raise EncryptionError(f"Failed to initialize encryption: {e}, fallback also failed: {fallback_error}")
    ...
    def _get_with_backend(self, key: str) -> Optional[Any]:
        ...
        try:
            ...
            if isinstance(decrypted_data, dict) and 'data' in decrypted_data:
                ...
                logger.warning(f"Cache entry for key {key} has unexpected format, returning raw data")
    ...
    def _set_with_backend(self, key: str, data: Any, ttl: Optional[int], operation: str) -> None:
        ...
        try:
            ...
            try:
                ...
                logger.debug(f"Cached data for key: {key} with TTL: {effective_ttl}s (backend: {type(self.backend).__name__}, encryption: {self.encryption_provider.get_encryption_type()})")
            except CacheBackendError as e:
    ...
    def _invalidate_with_backend(self, key: Optional[str]) -> None:
        ...
        try:
            ...
            if key is None:
                ...
                logger.debug(f"Invalidated cache entry for key: {key} (backend: {type(self.backend).__name__})")
        except CacheBackendError as e:
    ...
    def get_configuration_info(self) -> Dict[str, Any]:
        ...
        config_info = {
            ...
            "encryption_provider": type(self.encryption_provider).__name__ if self.encryption_provider else None,
            ...
            try:
                ...
                if isinstance(self.config, AdvancedCacheConfig):
                    backend_stats.update({
                        ...
                        'encryption_type': self.config.encryption_type if self.config.encryption_enabled else 'none'
                    })
    ...
    def _get_stats_legacy(self) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                'backend_type': 'file',
                'error': str(e),
                'cache_directory': str(self.path_manager.get_cache_directory()) if hasattr(self, 'path_manager') and self.path_manager else 'unknown'
            }
    ...
    def get_backend_info(self) -> Dict[str, Any]:
        ...
        info = {
            ...
            'enabled': self.config.enabled,
            'using_backend_system': self.backend is not None and self.encryption_provider is not None
        }
        ...
        if self.backend:
            try:
                ...
                health['checks']['backend'] = {
                    ...
                    'message': 'Backend is healthy' if backend_healthy else 'Backend health check failed'
                }
        ...
        if self.encryption_provider:
            try:
                ...
                health['checks']['encryption'] = {
                    ...
                    'message': 'Encryption provider is available' if encryption_available else 'Encryption provider not available'
                }
