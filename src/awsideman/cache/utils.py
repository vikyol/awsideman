...
class CachePathManager:
    ...
    def ensure_cache_directory(self) -> None:
        ...
        try:
            ...
            raise PermissionError(f"Permission denied creating cache directory {self.cache_dir}: {e}")
        except OSError as e:
    ...
    def clear_all_cache_files(self) -> int:
        ...
        if failed_count > 0:
            import logging
            logger = logging.getLogger(__name__)
            logger.warning(f"Failed to delete {failed_count} cache files out of {len(cache_files)} total")
