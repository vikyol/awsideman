...
class HybridBackend(CacheBackend):
    ...
    def __init__(self, 
                 ...
                 remote_backend: DynamoDBBackend,
                 local_ttl: int = 300):
        """
        ...
        try:
            ...
            if self._should_cache_locally(key, operation):
                try:
                    ...
                    logger.debug(f"Stored key {key} to local backend with TTL: {effective_local_ttl}s")
                except CacheBackendError as e:
                    local_error = e
    ...
    def invalidate(self, key: Optional[str] = None) -> None:
        ...
        try:
            ...
            if local_error and remote_error:
                logger.error(f"Both backends failed during invalidation: local={local_error}, remote={remote_error}")
                raise remote_error
            elif remote_error:
                raise remote_error
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        try:
            ...
            stats['cache_efficiency'] = {
                ...
                'local_hit_potential': f"{(local_entries / max(remote_entries, 1)) * 100:.1f}%" if remote_entries > 0 else "0%"
            }
    ...
    def health_check(self) -> bool:
        ...
        try:
            ...
            if is_healthy:
                logger.debug(f"Hybrid backend health check passed (local: {local_healthy}, remote: {remote_healthy})")
            else:
    ...
    def get_detailed_health_status(self) -> BackendHealthStatus:
        ...
        try:
            ...
            if local_status.is_healthy and remote_status.is_healthy:
                ...
                message = f"Local backend healthy, remote backend unhealthy: {remote_status.message}"
    ...
    def _cleanup_old_tracking_data(self) -> None:
        ...
        """
        current_time = time.time()
        cleanup_threshold = current_time - (self.local_ttl * 2)  # Clean up data older than 2x local TTL
    ...
    def _should_cache_locally(self, key: str, operation: str) -> bool:
        ...
        high_frequency_operations = {
            'list_roots', 'list_accounts', 'describe_account',
            'list_organizational_units', 'list_children',
    ...
    def sync_backends(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if local_healthy and remote_healthy:
                ...
                sync_result['errors'].append(f"Backend health issues - local: {local_healthy}, remote: {remote_healthy}")
