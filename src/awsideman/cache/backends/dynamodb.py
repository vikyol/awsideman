...
class DynamoDBBackend(CacheBackend):
    ...
    def __init__(self, 
        ...
        logger.debug(f"Initialized DynamoDB backend: table={table_name}, region={region}, profile={profile}")
    ...
    def invalidate(self, key: Optional[str] = None) -> None:
        ...
        try:
            ...
            if error_code == 'ResourceNotFoundException':
                ...
                logger.warning(f"DynamoDB throttling during invalidation")
    ...
    def validate_table_schema(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if len(key_schema) != 1:
                validation_result['errors'].append(f"Expected 1 key attribute, found {len(key_schema)}")
            elif key_schema[0].get('AttributeName') != 'cache_key' or key_schema[0].get('KeyType') != 'HASH':
            ...
            cache_key_attr = next((attr for attr in attributes if attr['AttributeName'] == 'cache_key'), None)
            ...
            if table_status != 'ACTIVE':
                validation_result['warnings'].append(f"Table status is {table_status}, expected ACTIVE")
    ...
    def get_table_info(self) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                'read_capacity': table_info.get('ProvisionedThroughput', {}).get('ReadCapacityUnits'),
                'write_capacity': table_info.get('ProvisionedThroughput', {}).get('WriteCapacityUnits'),
                'key_schema': table_info.get('KeySchema', []),
    ...
    def _decode_item_data(self, item: Dict[str, Any]) -> bytes:
        ...
        try:
            ...
            raise CacheBackendError(
                f"Failed to decode cache data: {e}",
                backend_type="dynamodb",
                original_error=e
            )
    ...
    def _get_chunked_data(self, key: str, metadata_item: Dict[str, Any]) -> Optional[bytes]:
        ...
        try:
            ...
            if len(chunks) != chunk_count:
                logger.warning(f"Missing chunks for key {key}: expected {chunk_count}, got {len(chunks)}")
                return None
    ...
    def _store_chunks_batch(self, chunks: List[Dict[str, Any]], metadata_item: Dict[str, Any]) -> None:
        """
        Store chunks and metadata using batch operations.
        ...
        try:
            ...
            raise CacheBackendError(
                f"Failed to store chunks in batch: {e}",
                backend_type="dynamodb",
                original_error=e
            )
    ...
    def _put_item_with_retry(self, item: Dict[str, Any]) -> None:
        ...
        try:
            ...
            if error_code == 'ResourceNotFoundException':
                ...
                logger.warning(f"DynamoDB throttling for put operation")
                # Don't raise error for throttling, just log and continue
            else:
                raise CacheBackendError(
                    f"DynamoDB put operation failed: {e}",
                    backend_type="dynamodb",
                    original_error=e
                )
