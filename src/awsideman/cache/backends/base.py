...
class CacheBackendError(Exception):
    ...
    def __init__(self, message: str, backend_type: str = "unknown", original_error: Optional[Exception] = None):
        """
        ...
        if original_error:
            logger.error(f"Cache backend error in {backend_type}: {message} (caused by: {original_error})")
        else:
