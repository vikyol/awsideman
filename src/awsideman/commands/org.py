...
def account(
    ...
    try:
        ...
        if not _is_valid_account_id(account_id):
            console.print(f"[red]Error: Invalid account ID format '{account_id}'. Account ID must be a 12-digit number.[/red]")
            raise typer.Exit(1)
...
def _output_account_json(account_details) -> None:
    ...
    account_dict = {
        ...
        "joined_timestamp": account_details.joined_timestamp.isoformat() if account_details.joined_timestamp else None,
...
def _output_search_results_json(matching_accounts: List) -> None:
    ...
    for account in matching_accounts:
        account_dict = {
            ...
            "joined_timestamp": account.joined_timestamp.isoformat() if account.joined_timestamp else None,
            "tags": account.tags,
...
def _output_policies_table(policies: List, account_id: str) -> None:
    ...
    if scps:
        ...
        for policy in scps:
            status_style = "green" if policy.effective_status == "ENABLED" else "yellow" if policy.effective_status == "CONDITIONAL" else "red"
            scp_table.add_row(
                policy.name,
                policy.id,
