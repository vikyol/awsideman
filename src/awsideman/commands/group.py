...
from ..utils.validators import (
    ...
    validate_limit, validate_group_name, validate_group_description
)
...
@app.command("list")
def list_groups(
    filter: Optional[str] = typer.Option(None, "--filter", "-f", help="Filter groups by attribute in format 'attribute=value' (e.g., DisplayName=engineering)"),
    limit: Optional[int] = typer.Option(None, "--limit", "-l", help="Maximum number of groups to return in a single page"),
    ...
    try:
        ...
        if re.match(uuid_pattern, group_identifier, re.IGNORECASE):
            ...
            try:
                    ...
                    Filters=[
                        {
                            ...
                            "AttributeValue": group_identifier
                        }
                    ]
                )
                ...
                if not groups:
                    console.print(f"[red]Error: No group found with name '{group_identifier}'.[/red]")
                    console.print("[yellow]Please check the group name and try again.[/yellow]")
                    console.print("[yellow]You can list available groups with 'awsideman group list'.[/yellow]")
                    raise typer.Exit(1)
                elif len(groups) > 1:
                    console.print(f"[yellow]Warning: Multiple groups found matching '{group_identifier}'. Using the first match.[/yellow]")
                    console.print("[yellow]Consider using the group ID instead of the name for more precise targeting.[/yellow]")
                
                # Use the first group found
                group_id = groups[0].get("GroupId")
                console.print(f"[green]Found group: {groups[0].get('DisplayName', 'N/A')} (ID: {group_id})[/green]")
                
            except ClientError as search_error:
                console.print(f"[red]Error searching for group: {search_error.response.get('Error', {}).get('Message', str(search_error))}[/red]")
                console.print("[yellow]Try using the group ID instead of the name if this error persists.[/yellow]")
        ...
        try:
            ...
            def list_memberships_with_retry():
                return identity_store.list_group_memberships(
                    **list_members_params
                )
            ...
            console.print(f"[green]Found {len(memberships)} members in group '{display_name}'{page_info}.[/green]")
            ...
            user_ids = [membership.get("MemberId", {}).get("UserId", "") for membership in memberships]
            user_details = {}
            ...
            for user_id in user_ids:
                ...
                try:
                    ...
                    console.print(f"[yellow]Warning: Could not retrieve details for user {user_id}: {e.response.get('Error', {}).get('Message', str(e))}[/yellow]")
...
def list_members(
    ...
    limit: Optional[int] = typer.Option(None, "--limit", "-l", help="Maximum number of members to return in a single page"),
    next_token: Optional[str] = typer.Option(None, "--next-token", "-n", help="Pagination token (for internal use)"),
    profile: Optional[str] = typer.Option(None, "--profile", "-p", help="AWS profile to use (uses default profile if not specified)"),
):
...
def add_member(
    ...
    user_identifier: str = typer.Argument(..., help="User ID, username, or email to add to the group"),
    profile: Optional[str] = typer.Option(None, "--profile", "-p", help="AWS profile to use (uses default profile if not specified)"),
):
...
def remove_member(
    ...
    user_identifier: str = typer.Argument(..., help="User ID, username, or email to remove from the group"),
...
def delete_group(
    ...
    try:
        ...
        if re.match(uuid_pattern, identifier, re.IGNORECASE):
            ...
            try:
                ...
                if not groups:
                    ...
                    console.print(f"[yellow]Warning: Multiple groups found matching '{identifier}'. Deleting the first match.[/yellow]")
                    console.print("[yellow]Consider using the group ID instead of the name for more precise targeting.[/yellow]")
        ...
        try:
            ...
            # Log the operation
            console.print(f"[blue]Preparing to delete group '{display_name}' (ID: {group_id}) from Identity Store {identity_store_id}...[/blue]")
        ...
        if not force:
            console.print(f"[yellow]Warning: This will permanently delete group '{display_name}' (ID: {group_id}).[/yellow]")
        ...
        try:
            ...
            if error_code == "ResourceNotFoundException":
                console.print(f"[red]Error: Group with ID '{group_id}' not found or already deleted.[/red]")
                console.print("[yellow]The group may have been deleted by another process or user.[/yellow]")
            else:
