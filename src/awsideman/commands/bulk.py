...
@app.command("assign")
def bulk_assign(
    input_file: Path = typer.Argument(..., help="Input file (CSV or JSON) with assignment data using human-readable names"),
    dry_run: bool = typer.Option(False, "--dry-run", help="Validate input and show preview without making changes"),
    continue_on_error: bool = typer.Option(True, "--continue-on-error/--stop-on-error", help="Continue processing on individual failures (default: continue)"),
    batch_size: int = typer.Option(10, "--batch-size", help="Number of assignments to process in parallel (1-50, default: 10)"),
    profile: Optional[str] = typer.Option(None, "--profile", help="AWS profile to use (uses default if not specified)"),
):
    ...
    try:
        ...
        from ..utils.bulk import (
            ...
            BatchProcessor, ReportGenerator, ValidationError
        )
        ...
        try:
            ...
            console.print(f"[green]✓ Detected file format: {FileFormatDetector.detect_format(input_file).upper()}[/green]")
        ...
        try:
            ...
            successful_resolutions = sum(1 for a in resolved_assignments if a.get('resolution_success', False))
            failed_resolutions = len(resolved_assignments) - successful_resolutions
            
            console.print(f"[green]✓ Successfully resolved {successful_resolutions} assignments[/green]")
            if failed_resolutions > 0:
                console.print(f"[yellow]⚠ {failed_resolutions} assignments had resolution errors[/yellow]")
        ...
        try:
            ...
            valid_assignments = [
                ...
                if a.get('resolution_success', False)
            ]
        ...
        try:
            ...
            # Process assignments
            results = asyncio.run(batch_processor.process_assignments(
                valid_assignments,
                "assign",
                instance_arn,
                dry_run=False,
        ...
        if results.failure_count == 0:
            ...
            console.print(f"[green]{results.success_count} assignments succeeded[/green], [red]{results.failure_count} failed[/red]")
        else:
            console.print(f"\n[red]❌ Bulk assign operation failed.[/red]")
            console.print(f"[red]No assignments were created successfully.[/red]")
            raise typer.Exit(1)
...
def bulk_revoke(
    ...
    continue_on_error: bool = typer.Option(True, "--continue-on-error/--stop-on-error", help="Continue processing on individual failures (default: continue)"),
    batch_size: int = typer.Option(10, "--batch-size", help="Number of assignments to process in parallel (1-50, default: 10)"),
    force: bool = typer.Option(False, "--force", "-f", help="Skip confirmation prompts and proceed automatically"),
    profile: Optional[str] = typer.Option(None, "--profile", help="AWS profile to use (uses default if not specified)"),
):
    ...
    try:
        ...
        try:
            ...
            if not preview_generator.prompt_user_confirmation("revoke", preview_summary, force=force):
        ...
        if results.failure_count == 0:
            ...
            console.print(f"[red]No assignments were revoked successfully.[/red]")
            raise typer.Exit(1)
