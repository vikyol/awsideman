    ...
    try:
        ...
        if accounts_only:
            ...
            if profile == "*":
                ...
                optimizer = AccountCacheOptimizer(None, cache_manager, profile="*")
                ...
                optimizer = AccountCacheOptimizer(None, cache_manager, profile=profile)
                cleared_count = optimizer.force_clear_all_account_cache()
    ...
    try:
        ...
        if isinstance(cache_manager.config, AdvancedCacheConfig):
            ...
            if config.backend_type == "dynamodb":
                ...
                console.print(
                    f"[green]DynamoDB Region:[/green] {config.dynamodb_region or 'default'}"
                )
                console.print(
                    f"[green]DynamoDB Profile:[/green] {config.dynamodb_profile or 'default'}"
                )
            elif config.backend_type == "file":
                console.print(
                    f"[green]File Cache Directory:[/green] {config.file_cache_dir or 'default (~/.awsideman/cache)'}"
                )
            ...
            console.print("[green]Backend Type:[/green] file (legacy)")
            console.print(
                "[dim]Using basic cache configuration - advanced features not available[/dim]"
            )
...
def _display_encryption_status(cache_manager: CacheManager) -> None:
    ...
    try:
        ...
        if isinstance(cache_manager.config, AdvancedCacheConfig):
            config = cache_manager.config
            console.print(
                f"[green]Encryption Enabled:[/green] {'Yes' if config.encryption_enabled else 'No'}"
            )
            ...
            console.print("[green]Encryption Enabled:[/green] No")
...
def _display_backend_statistics(cache_manager: CacheManager) -> None:
    ...
    try:
        ...
        if hasattr(cache_manager, "backend") and cache_manager.backend:
            ...
            console.print(
                "[dim]Backend statistics not available (using legacy configuration)[/dim]"
            )
...
def _display_dynamodb_statistics(stats: Dict[str, Any]) -> None:
    ...
    console.print(
        f"[{exists_color}]Table Exists:[/{exists_color}] {'Yes' if table_exists else 'No'}"
    )
    ...
    if table_exists:
        ...
        console.print(
            f"[green]Chunking Enabled:[/green] {'Yes' if stats.get('chunking_enabled', False) else 'No'}"
        )
        console.print(
            f"[green]Compression Enabled:[/green] {'Yes' if stats.get('compression_enabled', False) else 'No'}"
        )
...
def _display_hybrid_statistics(stats: Dict[str, Any]) -> None:
    ...
    console.print(
        f"[green]Local Hit Potential:[/green] {cache_efficiency.get('local_hit_potential', '0%')}"
    )
...
def _display_recent_cache_entries(cache_manager: CacheManager) -> None:
    ...
    try:
            ...
            try:
                ...
                try:
                    ...
                    if not all(field in cache_data for field in required_fields):
                        console.print(
                            f"[yellow]Warning: Cache file missing required fields: {cache_file.name}[/yellow]"
                        )
                        continue
                    ...
                    try:
                        ...
                        console.print(
                            f"[yellow]Warning: Could not process cache file: {cache_file.name}[/yellow]"
                        )
                        continue
                ...
                console.print(
                    f"[yellow]Warning: Could not read cache file {cache_file.name}: {e}[/yellow]"
                )
                continue
        ...
        for cache_entry, file_size_kb in cache_entries[:10]:
            ...
            table.add_row(operation_display, age_str, ttl_str, status, size_str)
...
def encryption_status():
    ...
    try:
        ...
        if not isinstance(cache_manager.config, AdvancedCacheConfig):
            console.print(
                "[yellow]Using basic cache configuration - encryption not available[/yellow]"
            )
            console.print(
                "[dim]To use encryption, configure advanced cache settings in ~/.awsideman/config.yaml[/dim]"
            )
            return
        ...
        if config.encryption_enabled and config.encryption_type != "none":
            ...
            try:
                ...
                console.print(
                    f"[{keyring_color}]Keyring Status:[/{keyring_color}] {keyring_status}"
                )
                ...
                # Display service information
                console.print(
                    f"[green]Service Name:[/green] {key_info.get('service_name', 'unknown')}"
                )
        ...
        if not config.encryption_enabled:
            console.print(
                "[yellow]• Consider enabling encryption for sensitive cache data[/yellow]"
            )

        if config.encryption_enabled and not key_info.get("keyring_available", False):
            console.print(
                "[yellow]• Keyring unavailable - using less secure file-based key storage[/yellow]"
            )
...
def enable_encryption(
    ...
    encryption_type: str = typer.Option(
        "aes256", "--type", "-t", help="Encryption type to use (aes256)"
    ),
):
    ...
    try:
        ...
        if not isinstance(cache_manager.config, AdvancedCacheConfig):
            ...
            console.print(
                "[dim]Configure advanced cache settings in ~/.awsideman/config.yaml[/dim]"
            )
            raise typer.Exit(1)
        ...
        if total_entries == 0:
            ...
            console.print(
                f"[yellow]About to enable encryption and re-encrypt {total_entries} cache entries[/yellow]"
            )
        ...
        if (
            not new_cache_manager.encryption_provider
            or new_cache_manager.encryption_provider.get_encryption_type() == "none"
        ):
        ...
        if total_entries > 0:
            ...
            console.print(
                "[yellow]Clearing existing cache - it will be rebuilt with encryption[/yellow]"
            )
        ...
        try:
            ...
            if encryption_type != "none":
                ...
                if key_info.get("keyring_available", False):
                    console.print(
                        "[green]✓ Encryption key stored securely in system keyring[/green]"
                    )
                else:
                    console.print(
                        "[yellow]⚠ Encryption key stored in file (keyring unavailable)[/yellow]"
                    )
...
def disable_encryption(
    ...
    keep_data: bool = typer.Option(
        True, "--keep-data/--clear-data", help="Keep encrypted data or clear it"
    ),
):
    ...
    try:
        ...
        if not force:
            ...
            if keep_data:
                ...
                console.print(
                    "• You will need to re-enable encryption with the same key to access the data"
                )
            else:
        ...
        try:
            ...
            if key_manager.key_exists():
                if key_manager.delete_key():
                    ...
                    console.print(
                        "[yellow]⚠ Failed to delete encryption key - you may need to remove it manually[/yellow]"
                    )
        except Exception as e:
        ...
        try:
            ...
            if keep_data and total_entries > 0:
                console.print(
                    f"[yellow]Note: {total_entries} encrypted cache entries are still present but inaccessible[/yellow]"
                )
...
def rotate_encryption_key(
    ...
    backup_old_key: bool = typer.Option(
        True, "--backup/--no-backup", help="Backup old key before rotation"
    ),
):
    ...
    try:
        ...
        if not isinstance(cache_manager.config, AdvancedCacheConfig):
            console.print(
                "[red]Error: Advanced cache configuration required for key rotation[/red]"
            )
            raise typer.Exit(1)
        ...
        if not config.encryption_enabled or config.encryption_type == "none":
            ...
            console.print(
                "[dim]Use 'awsideman cache encryption enable' to enable encryption first[/dim]"
            )
            raise typer.Exit(1)

        # Check that encryption is working
        if (
            not cache_manager.encryption_provider
            or cache_manager.encryption_provider.get_encryption_type() == "none"
        ):
        ...
        console.print(
            f"[yellow]About to rotate encryption key and re-encrypt {total_entries} cache entries[/yellow]"
        )
        ...
        try:
            ...
            if not old_key:
                console.print(
                    "[yellow]No old key found - this appears to be the first key generation[/yellow]"
                )
            ...
            if total_entries > 0:
                console.print(
                    f"[blue]Re-encrypting {total_entries} cache entries with new key...[/blue]"
                )
                ...
                console.print(
                    "[yellow]Clearing cache - it will be rebuilt with the new key[/yellow]"
                )
                cache_manager.invalidate()

                console.print(
                    "[green]✓ Cache cleared - will be rebuilt with new encryption key[/green]"
                )
...
def backup_encryption_key(
    output_file: Optional[str] = typer.Option(
        None, "--output", "-o", help="Output file for key backup"
    ),
...
def restore_encryption_key(
    ...
    try:
        ...
        if missing_fields:
            console.print(
                f"[red]Error: Backup file missing required fields: {', '.join(missing_fields)}[/red]"
            )
            raise typer.Exit(1)
        ...
        key_manager = KeyManager(
            service_name=backup_data["service_name"], username=backup_data["username"]
        )
...
def health_check(
    ...
    backend: Optional[str] = typer.Option(
        None, "--backend", "-b", help="Check specific backend (file, dynamodb, hybrid)"
    ),
):
    ...
    try:
        ...
        console.print(f"[green]Backend Type:[/green] {config.backend_type}")
        console.print(
            f"[green]Encryption:[/green] {'Enabled' if config.encryption_enabled else 'Disabled'}"
        )

        # Check specific backend if requested
        if backend and backend != config.backend_type:
            console.print(
                f"[yellow]Note: Checking {backend} backend (not currently active)[/yellow]"
            )
        ...
        if overall_healthy:
            ...
            console.print(
                "[dim]Use 'awsideman cache health repair' to attempt automatic fixes[/dim]"
            )
...
def benchmark_backends(
    operations: int = typer.Option(
        100, "--operations", "-n", help="Number of operations to perform"
    ),
    data_size: int = typer.Option(1024, "--size", "-s", help="Size of test data in bytes"),
    backend: Optional[str] = typer.Option(
        None, "--backend", "-b", help="Benchmark specific backend"
    ),
):
    ...
    try:
        ...
        if not isinstance(cache_manager.config, AdvancedCacheConfig):
            console.print(
                "[yellow]Performance benchmarking requires advanced cache configuration[/yellow]"
            )
            return

        config = cache_manager.config

        console.print("[green]Test Parameters:[/green]")
...
def repair_backends(
    ...
    try:
        ...
        if not force:
            console.print(
                f"[yellow]About to attempt repair of {', '.join(backends_to_repair)} backend(s)[/yellow]"
            )
...
def test_connectivity(
    backend: Optional[str] = typer.Option(
        None, "--backend", "-b", help="Test specific backend connectivity"
    ),
    ...
    try:
        ...
        if not isinstance(cache_manager.config, AdvancedCacheConfig):
            console.print(
                "[yellow]Connectivity testing requires advanced cache configuration[/yellow]"
            )
            return
...
def _benchmark_backend(
    backend_type: str, config: AdvancedCacheConfig, test_data: bytes, operations: int
) -> dict:
        ...
        if backend_type == "file":
            ...
            if backend.health_check():
                ...
                try:
                    ...
                    if backend.health_check():
                        ...
                        console.print(
                            "[red]✗ File backend still unhealthy after repair attempt[/red]"
                        )
                        return False
                except Exception as e:
            ...
            if hasattr(backend, "repair_table"):
                ...
                if backend.health_check():
                    ...
                    console.print(
                        "[red]✗ DynamoDB backend is unhealthy and cannot be repaired automatically[/red]"
                    )
                    return False
...
def _test_backend_connectivity(
    backend_type: str, config: AdvancedCacheConfig, timeout: int
) -> bool:
            ...
            try:
                ...
                if hasattr(backend, "get_table_info"):
                    ...
                    if table_info.get("exists", False):
                        console.print(
                            f"[green]✓ Table '{config.dynamodb_table_name}' is accessible[/green]"
                        )
                        console.print(
                            f"[green]  Status: {table_info.get('table_status', 'Unknown')}[/green]"
                        )
                    else:
                        console.print(
                            f"[yellow]⚠ Table '{config.dynamodb_table_name}' does not exist[/yellow]"
                        )
                        console.print(
                            "[dim]  Table will be created automatically when needed[/dim]"
                        )
    ...
    try:
        ...
        if isinstance(cache_manager.config, AdvancedCacheConfig):
            ...
            if config.backend_type in ["file", "hybrid"]:
                console.print(
                    f"[green]File Cache Dir:[/green] {config.file_cache_dir or 'default'}"
                )
            ...
            console.print("[yellow]Using basic cache configuration[/yellow]")
            console.print(
                f"[green]Enabled:[/green] {'Yes' if cache_manager.config.enabled else 'No'}"
            )
...
def set_backend(
    ...
    local_ttl: Optional[int] = typer.Option(
        None, "--local-ttl", help="Hybrid local TTL in seconds"
    ),
    ...
    try:
        ...
        try:
            ...
            if not provider.is_available():
                console.print(
                    f"[red]Error: Encryption provider '{encryption_type}' is not available[/red]"
                )
                raise typer.Exit(1)
        except Exception as e:
...
def create_dynamodb_table(
    ...
    try:
        ...
        backend = DynamoDBBackend(table_name=table_name, region=region, profile=profile)
        ...
        try:
            ...
            console.print(f"[green]Item Count:[/green] {stats.get('item_count', 0)}")
            console.print(
                f"[green]TTL Enabled:[/green] {'Yes' if stats.get('ttl_enabled', False) else 'No'}"
            )
            return
        ...
        if stats.get("table_exists", False):
            ...
            console.print(
                f"[green]✓ TTL Enabled: {'Yes' if stats.get('ttl_enabled', False) else 'No'}[/green]"
            )
...
def test_configuration():
    ...
    try:
        ...
        if (
            isinstance(cache_manager.config, AdvancedCacheConfig)
            and cache_manager.config.encryption_enabled
        ):
            console.print("\n[blue]Testing encryption...[/blue]")
            try:
                if (
                    hasattr(cache_manager, "encryption_provider")
                    and cache_manager.encryption_provider
                ):
                    is_available = cache_manager.encryption_provider.is_available()
                    if is_available:
    ...
    try:
        ...
        if config.backend_type == "file" and not config.encryption_enabled:
            recommendations.append(
                "Consider enabling encryption for file-based cache to protect sensitive data"
            )

        if config.backend_type == "dynamodb" and not config.dynamodb_region:
            recommendations.append(
                "Specify a DynamoDB region for better performance and cost control"
            )

        # TTL recommendations
        if config.default_ttl > 86400:  # 24 hours
            recommendations.append(
                "Default TTL is quite long - consider shorter TTL for more up-to-date data"
            )
        ...
        if config.max_size_mb > 1000:  # 1GB
            ...
            recommendations.append(
                "Max cache size is quite small - may result in frequent cache evictions"
            )
...
def account_cache_status(
    profile: Optional[str] = typer.Option(None, "--profile", help="AWS profile to use")
):
    """Display account cache status and statistics.

    Shows detailed information about account-related cache entries including:
    - Organization snapshot cache status and age
    ...
    try:
        ...
        # Initialize clients
        client_manager = AWSClientManager(profile=profile)
        organizations_client = client_manager.get_organizations_client()
        cache_manager = CacheManager()

        # Normalize profile name - use "default" when no profile is specified
        if profile is None:
            profile = "default"

        # Create optimizer to get account-specific stats
        optimizer = AccountCacheOptimizer(organizations_client, cache_manager, profile=profile)
        account_stats = optimizer.get_cache_stats()

        console.print("[bold blue]Account Cache Status[/bold blue]")
        console.print("=" * 50)

        # Organization snapshot status
        if account_stats["org_snapshot_cached"]:
            age_seconds = account_stats["org_snapshot_age_seconds"]
            age_hours = age_seconds // 3600
            age_minutes = (age_seconds % 3600) // 60

            console.print("[green]Organization Snapshot:[/green] Cached")
            console.print(
                f"[green]Snapshot Age:[/green] {age_hours}h {age_minutes}m ({age_seconds}s)"
            )

            # Show freshness indicator
            if age_seconds < 3600:  # Less than 1 hour
                freshness = "[green]Very Fresh[/green]"
            elif age_seconds < 12 * 3600:  # Less than 12 hours
                freshness = "[yellow]Fresh[/yellow]"
            elif age_seconds < 24 * 3600:  # Less than 24 hours
                freshness = "[orange]Aging[/orange]"
            else:
                freshness = "[red]Stale[/red]"

            console.print(f"[green]Freshness:[/green] {freshness}")
        else:
            console.print("[red]Organization Snapshot:[/red] Not cached")

        # Account count status
        if account_stats["account_count_cached"]:
            console.print("[green]Account Count:[/green] Cached")
        else:
            console.print("[red]Account Count:[/red] Not cached")
        ...
        if not account_stats["org_snapshot_cached"]:
            console.print(
                "• [yellow]Organization snapshot not cached - first wildcard filter will be slow[/yellow]"
            )
            console.print("• [blue]Run a wildcard filter operation to populate the cache[/blue]")
        elif (
            account_stats["org_snapshot_age_seconds"]
            and account_stats["org_snapshot_age_seconds"] > 20 * 3600
        ):
            console.print(
                "• [yellow]Organization snapshot is getting old - consider refreshing[/yellow]"
            )
            console.print(
                "• [blue]Use 'awsideman cache clear --accounts-only' to refresh account data[/blue]"
            )
        else:
            console.print("• [green]Account cache is optimally configured[/green]")

        # Show cache configuration recommendations
        console.print()
        console.print("[bold blue]Cache Configuration[/bold blue]")
        console.print("-" * 30)

        general_stats = cache_manager.get_cache_stats()
        if general_stats.get("enabled", False):
            console.print("[green]Cache Status:[/green] Enabled")

            # Check if organization operations have long TTLs
            config = cache_manager.config
            org_ops = [
                "list_roots",
            ...
            short_ttl_ops = []
            for op in org_ops:
                # Use direct attribute access since AdvancedCacheConfig might not inherit the method properly
                ttl = config.operation_ttls.get(op, config.default_ttl)
                if ttl < 12 * 3600:  # Less than 12 hours
                    short_ttl_ops.append(f"{op} ({ttl}s)")

            if short_ttl_ops:
                console.print(
                    "[yellow]Warning:[/yellow] Some organization operations have short TTLs:"
                )
                for op in short_ttl_ops:
                    console.print(f"  • {op}")
                console.print(
                    "[blue]Consider increasing TTLs for organization operations to 24h+[/blue]"
                )
            else:
                console.print("[green]Organization operation TTLs:[/green] Optimally configured")
        else:
            console.print("[red]Cache Status:[/red] Disabled")
            console.print(
                "[yellow]Enable caching for significant performance improvements[/yellow]"
            )

    except Exception as e:
        console.print(f"[red]Error getting account cache status: {e}[/red]")
        raise typer.Exit(1)
...
def inspect_cache(
    profile: Optional[str] = typer.Option(
        None, "--profile", help="AWS profile to inspect cache for"
    )
):
    """Inspect cache contents to understand cache structure.

    This diagnostic command shows what cache entries actually exist,
    which is helpful for troubleshooting cache clearing issues.
    """
    try:
        from ..cache.manager import CacheManager
        from ..utils.account_cache_optimizer import AccountCacheOptimizer
        ...
        # Create optimizer for inspection
        optimizer = AccountCacheOptimizer(None, cache_manager, profile=profile)
        inspection = optimizer.inspect_cache_keys()

        console.print("[bold blue]Cache Inspection Results[/bold blue]")
        console.print("=" * 50)

        console.print(f"[green]Total cache entries:[/green] {inspection['total_entries']}")

        # Show cache backend information
        try:
            if hasattr(cache_manager, "backend") and cache_manager.backend:
                backend_type = type(cache_manager.backend).__name__
                console.print(f"[green]Cache backend:[/green] {backend_type}")
            else:
                console.print("[yellow]Cache backend:[/yellow] Legacy/Unknown")
        except Exception:
            pass

        if "error" in inspection:
            console.print(f"[red]Error during inspection:[/red] {inspection['error']}")
            return

        if inspection.get("enumeration_failed"):
            console.print(
                f"[yellow]Warning:[/yellow] {inspection.get('message', 'Could not enumerate cache keys')}"
            )
        else:
            console.print(
                f"[green]Account-related keys found:[/green] {len(inspection['account_related_keys'])}"
            )
            console.print(f"[green]Other keys found:[/green] {len(inspection['other_keys'])}")

            if inspection["sample_keys"]:
                console.print("\n[bold]Sample cache keys:[/bold]")
                for i, key in enumerate(inspection["sample_keys"][:5], 1):
                    console.print(f"  {i}. {key}")

                if len(inspection["sample_keys"]) > 5:
                    console.print(f"  ... and {len(inspection['sample_keys']) - 5} more")

            if inspection["key_patterns"]:
                console.print("\n[bold]Key patterns:[/bold]")
                for pattern, count in sorted(
                    inspection["key_patterns"].items(), key=lambda x: x[1], reverse=True
                ):
                    console.print(f"  {pattern}: {count} entries")

            if inspection["account_related_keys"]:
                console.print("\n[bold]Account-related keys:[/bold]")
                for key in inspection["account_related_keys"][:10]:
                    console.print(f"  • {key}")

                if len(inspection["account_related_keys"]) > 10:
                    console.print(f"  ... and {len(inspection['account_related_keys']) - 10} more")

        console.print(
            "\n[dim]Use this information to understand why --accounts-only might not be working[/dim]"
        )
