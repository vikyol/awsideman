...
class ResourceResolver:
    ...
    def __init__(self, aws_client_manager: AWSClientManager, instance_arn: str, identity_store_id: str):
        ...
        try:
            ...
            if not users:
                return ResolutionResult(
                    success=False,
                    error_message=f"User '{user_name}' not found in Identity Store"
                )
            
            if len(users) > 1:
                return ResolutionResult(
                    success=False,
                    error_message=f"Multiple users found with name '{user_name}'"
                )
    ...
    def _resolve_group_name(self, group_name: str) -> ResolutionResult:
        ...
        try:
            ...
            if not groups:
                return ResolutionResult(
                    success=False,
                    error_message=f"Group '{group_name}' not found in Identity Store"
                )
            
            if len(groups) > 1:
                return ResolutionResult(
                    success=False,
                    error_message=f"Multiple groups found with name '{group_name}'"
                )
        ...
        try:
            ...
            for permission_set_arn in permission_set_arns:
                try:
                    ...
                    console.print(f"[yellow]Warning: Could not describe permission set {permission_set_arn}: {str(e)}[/yellow]")
                    continue
            
            # Permission set not found
            result = ResolutionResult(
                success=False,
                error_message=f"Permission set '{permission_set_name}' not found"
            )
            self._permission_set_cache[permission_set_name] = result
            return result
            ...
            def extract_accounts(node):
                ...
                if node.is_account():
                    ...
                    try:
                        ...
                        console.print(f"[yellow]Warning: Could not get details for account {node.id}: {str(e)}[/yellow]")
    ...
    def warm_cache_for_assignments(self, assignments: List[Dict[str, Any]]):
        ...
        if account_names and not self._account_name_to_id_cache:
            try:
                ...
                console.print(f"[yellow]Warning: Could not pre-warm account cache: {str(e)}[/yellow]")
        ...
        for ps_name in permission_set_names:
            if ps_name not in self._permission_set_cache:
                try:
                    ...
                    console.print(f"[yellow]Warning: Could not pre-warm permission set '{ps_name}': {str(e)}[/yellow]")
        ...
        for principal_name, principal_type in principal_names:
            ...
            if cache_key not in self._principal_cache:
                try:
                    ...
                    console.print(f"[yellow]Warning: Could not pre-warm principal '{principal_name}': {str(e)}[/yellow]")
...
class AssignmentValidator:
    ...
    def validate_assignment(self, assignment: Dict[str, Any]) -> List[str]:
        ...
        if principal_id:
            if not self.validate_principal(principal_id, principal_type):
                errors.append(f"Principal {principal_id} ({principal_type}) does not exist or is not accessible")
        
        # Validate permission set exists
        if permission_set_arn:
            if not self.validate_permission_set(permission_set_arn):
                errors.append(f"Permission set {permission_set_arn} does not exist or is not accessible")
    ...
    def validate_principal(self, principal_id: str, principal_type: str) -> bool:
        ...
        try:
            ...
            console.print(f"[yellow]Warning: Error validating principal {principal_id}: {str(e)}[/yellow]")
            return False
        except Exception as e:
            console.print(f"[yellow]Warning: Unexpected error validating principal {principal_id}: {str(e)}[/yellow]")
            return False
    ...
    def validate_permission_set(self, permission_set_arn: str) -> bool:
        ...
        try:
            ...
            if e.response['Error']['Code'] in ['ResourceNotFoundException', 'ValidationException']:
                return False
            # For other errors, log and return False
            console.print(f"[yellow]Warning: Error validating permission set {permission_set_arn}: {str(e)}[/yellow]")
            return False
        except Exception as e:
            console.print(f"[yellow]Warning: Unexpected error validating permission set {permission_set_arn}: {str(e)}[/yellow]")
            return False
    ...
    def validate_account(self, account_id: str) -> bool:
        ...
        try:
            ...
            if e.response['Error']['Code'] in ['AccountNotFoundException', 'ValidationException']:
                return False
            # For other errors, log and return False
            console.print(f"[yellow]Warning: Error validating account {account_id}: {str(e)}[/yellow]")
            return False
        except Exception as e:
            console.print(f"[yellow]Warning: Unexpected error validating account {account_id}: {str(e)}[/yellow]")
            return False
    
    def check_assignment_exists(self, principal_id: str, permission_set_arn: str, account_id: str) -> bool:
        ...
        try:
            ...
            console.print(f"[yellow]Warning: Error checking assignment existence: {str(e)}[/yellow]")
            return False
        except Exception as e:
            console.print(f"[yellow]Warning: Unexpected error checking assignment existence: {str(e)}[/yellow]")
            return False