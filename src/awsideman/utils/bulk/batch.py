    ...
    async def execute_with_retry(self, func: Callable, *args, **kwargs) -> Any:
        ...
        for attempt in range(self.max_retries + 1):
            try:
                ...
                console.print(f"[yellow]Retry attempt {attempt + 1}/{self.max_retries} after {delay:.1f}s: {str(e)}[/yellow]")
...
class BatchProcessor:
        ...
        try:
            ...
            for i in range(0, len(assignments), self.batch_size):
                ...
                if not continue_on_error and batch_results['failed']:
                    console.print(f"[red]Stopping processing due to failures (continue_on_error=False)[/red]")
                    break
        ...
        with ThreadPoolExecutor(max_workers=self.batch_size) as executor:
            ...
            for assignment in batch:
                try:
                    future = executor.submit(
                        ...
                        assignment, operation, instance_arn, dry_run
                    )
                    future_to_assignment[future] = assignment
                except Exception as e:
                    # Handle submission errors
                    console.print(f"[red]Error submitting assignment for processing: {str(e)}[/red]")
                    error_result = self._create_error_result(assignment, f"Submission error: {str(e)}")
                    batch_results['failed'].append(error_result)
                    
                    if not continue_on_error:
                        console.print(f"[red]Batch processing stopped due to submission error[/red]")
                        break
            ...
            for future in as_completed(future_to_assignment):
                ...
                try:
                    ...
                    console.print(f"[red]{error_msg} for assignment: {assignment.get('principal_name', 'unknown')}[/red]")
                    error_result = self._create_error_result(assignment, error_msg)
                    batch_results['failed'].append(error_result)
                    
                    if not continue_on_error:
                        console.print(f"[red]Batch processing stopped due to timeout[/red]")
                        break
                    ...
                    console.print(f"[dim]Assignment context: {principal_name} -> {permission_set_name} @ {account_name}[/dim]")
                    
                    error_result = self._create_error_result(assignment, error_msg)
                    batch_results['failed'].append(error_result)
                    
                    if not continue_on_error:
                        console.print(f"[red]Batch processing stopped due to error (processed {completed_count}/{len(future_to_assignment)})[/red]")
                        break
    ...
    def _process_single_assignment(
        ...
        if not assignment.get('resolution_success', False):
            ...
            error_message = '; '.join(resolution_errors) if resolution_errors else 'Resolution failed'
    ...
    def _create_error_result(self, assignment: Dict[str, Any], error_message: str) -> AssignmentResult:
    ...
    def _format_aws_error(self, error: Exception, operation: str = 'operation') -> str:
        ...
        if isinstance(error, ClientError):
            ...
            if error_code == 'AccessDeniedException':
                ...
                return f"Rate limit exceeded during {operation} operation. Will retry automatically."
        ...
        for attempt in range(self.retry_handler.max_retries + 1):
            try:
                ...
                matching_assignments = [
                    ...
                    if assignment.get("PrincipalId") == principal_id and 
                       assignment.get("PrincipalType") == principal_type
                ]
                ...
                if response.get('AccountAssignmentCreationStatus', {}).get('Status') == 'SUCCEEDED':
                ...
                elif response.get('AccountAssignmentCreationStatus', {}).get('Status') == 'IN_PROGRESS':
                    ...
                    failure_reason = response.get('AccountAssignmentCreationStatus', {}).get('FailureReason', 'Unknown failure')
                ...
                if attempt < self.retry_handler.max_retries and self.retry_handler.should_retry(e):
                    ...
                    console.print(f"[yellow]Retrying assign operation (attempt {attempt + 1}/{self.retry_handler.max_retries + 1}) after {delay:.1f}s: {str(e)}[/yellow]")
                    import time
                    time.sleep(delay)
                    continue
    ...
    def _execute_revoke_operation(
        ...
        for attempt in range(self.retry_handler.max_retries + 1):
            try:
                ...
                if response.get('AccountAssignmentDeletionStatus', {}).get('Status') == 'SUCCEEDED':
                ...
                elif response.get('AccountAssignmentDeletionStatus', {}).get('Status') == 'IN_PROGRESS':
                    # For IN_PROGRESS status, we consider it successful as the operation was accepted
                    return {
                        'status': 'success',
                    ...
                    # Handle failed status
                    failure_reason = response.get('AccountAssignmentDeletionStatus', {}).get('FailureReason', 'Unknown failure')
                ...
                if attempt < self.retry_handler.max_retries and self.retry_handler.should_retry(e):
                    retry_count += 1
                    delay = self.retry_handler.calculate_delay(attempt)
                    console.print(f"[yellow]Retrying revoke operation (attempt {attempt + 1}/{self.retry_handler.max_retries + 1}) after {delay:.1f}s: {str(e)}[/yellow]")
                    import time
                    time.sleep(delay)
                    continue
