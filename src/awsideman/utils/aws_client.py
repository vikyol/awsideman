    ...
    def _init_session(self):
        ...
        if self.region:
            ...
            try:
                ...
                console.print(f"[yellow]Warning: Could not get region from profile: {str(e)}[/yellow]")
...
def build_organization_hierarchy(organizations_client: OrganizationsClient) -> OrganizationTree:
    ...
    try:
        ...
        if not roots_data:
            raise ValueError("No organization roots found. This may indicate the account is not part of an organization.")
        ...
        for root_data in roots_data:
            try:
                ...
                console.print(f"[yellow]Warning: Failed to build hierarchy for root {root_data.get('Id', 'unknown')}: {str(e)}[/yellow]")
                # Continue with other roots if one fails
                continue
        
        if not organization_tree:
            raise ValueError("Failed to build organization hierarchy. No valid roots could be processed.")
        ...
        console.print(f"[red]Error: Unexpected error building organization hierarchy: {str(e)}[/red]")
        raise
    ...
    try:
        ...
        for ou_data in ous_data:
            try:
                ...
                console.print(f"[yellow]Warning: Failed to process OU {ou_data.get('Id', 'unknown')}: {str(e)}[/yellow]")
        ...
        for account_data in accounts_data:
            try:
                ...
                console.print(f"[yellow]Warning: Failed to process account {account_data.get('Id', 'unknown')}: {str(e)}[/yellow]")
                # Continue with other accounts if one fails
                continue
                
    except ClientError as e:
        console.print(f"[yellow]Warning: Failed to retrieve children for {parent_node.id}: {str(e)}[/yellow]")
        # Don't re-raise here as we want to continue building the rest of the tree
    except Exception as e:
        console.print(f"[yellow]Warning: Unexpected error building children for {parent_node.id}: {str(e)}[/yellow]")
...
def get_account_details(organizations_client: OrganizationsClient, account_id: str) -> AccountDetails:
    ...
    try:
        ...
        try:
            ...
            console.print(f"[yellow]Warning: Could not retrieve tags for account {account_id}: {str(e)}[/yellow]")
            tags = {}
        ...
        console.print(f"[red]Error: Failed to retrieve account details for {account_id}: {str(e)}[/red]")
        raise
    except Exception as e:
        console.print(f"[red]Error: Unexpected error retrieving account details for {account_id}: {str(e)}[/red]")
        raise
    ...
    try:
        ...
        for account_data in all_accounts:
            ...
            try:
                ...
                console.print(f"[yellow]Warning: Could not get details for account {account_id}: {str(e)}[/yellow]")
                continue
...
def _account_matches_tag_filter(account_details: AccountDetails, tag_filter: Dict[str, str]) -> bool:
    """
...
def _calculate_ou_path(organizations_client: OrganizationsClient, account_id: str) -> List[str]:
    ...
    try:
        ...
        while True:
            ...
            if parent_type == 'ROOT':
                ...
                try:
                    ...
                    root_name = next((root['Name'] for root in roots if root['Id'] == parent_id), parent_id)
                    path.append(root_name)
                except Exception as e:
                    console.print(f"[yellow]Warning: Could not get root name for {parent_id}: {str(e)}[/yellow]")
            ...
            # If parent is an OU, get its name and continue up the hierarchy
                ...
                try:
                    ...
                    if grandparents:
                        ...
                        ous = organizations_client.list_organizational_units_for_parent(grandparent_id)
                        ou_name = next((ou['Name'] for ou in ous if ou['Id'] == parent_id), parent_id)
                        path.append(ou_name)
                    else:
                        path.append(parent_id)
                except Exception as e:
                    console.print(f"[yellow]Warning: Could not get OU name for {parent_id}: {str(e)}[/yellow]")
                    path.append(parent_id)
        ...
        console.print(f"[yellow]Warning: Failed to calculate OU path for {account_id}: {str(e)}[/yellow]")
        return []
    except Exception as e:
        console.print(f"[yellow]Warning: Unexpected error calculating OU path for {account_id}: {str(e)}[/yellow]")
        return []
    ...
    def resolve_policies_for_account(self, account_id: str) -> PolicyList:
        ...
        try:
            ...
            for i, target_id in enumerate(hierarchy_path.ids):
                target_name = hierarchy_path.names[i] if i < len(hierarchy_path.names) else target_id
                target_type = hierarchy_path.types[i] if i < len(hierarchy_path.types) else NodeType.OU
                ...
                try:
                    ...
                    console.print(f"[yellow]Warning: Could not get policies for {target_id} ({target_name}): {str(e)}[/yellow]")
                    continue
            
            return all_policies
            
        except ClientError as e:
            console.print(f"[red]Error: Failed to resolve policies for account {account_id}: {str(e)}[/red]")
            raise
        except Exception as e:
            console.print(f"[red]Error: Unexpected error resolving policies for account {account_id}: {str(e)}[/red]")
            raise
    ...
    def _get_hierarchy_path(self, account_id: str) -> HierarchyPath:
        ...
        try:
            ...
            try:
                ...
                console.print(f"[yellow]Warning: Could not get account details for {account_id}: {str(e)}[/yellow]")
                path_ids.append(account_id)
            ...
            while True:
                ...
                if parent_type == 'ROOT':
                    ...
                    try:
                        ...
                        if grandparents:
                            grandparent_id = grandparents[0]['Id']
                            ous = self.organizations_client.list_organizational_units_for_parent(grandparent_id)
                            ou_name = next((ou['Name'] for ou in ous if ou['Id'] == parent_id), parent_id)
                            path_ids.append(parent_id)
            ...
            console.print(f"[yellow]Warning: Failed to get hierarchy path for {account_id}: {str(e)}[/yellow]")
            return HierarchyPath(ids=[], names=[], types=[])
        except Exception as e:
            console.print(f"[yellow]Warning: Unexpected error getting hierarchy path for {account_id}: {str(e)}[/yellow]")
            return HierarchyPath(ids=[], names=[], types=[])
        ...
        try:
            ...
            for policy_data in policy_data_list:
                try:
                    ...
                    console.print(f"[yellow]Warning: Could not process policy data for {target_id}: {str(e)}[/yellow]")
                    continue
            
        except ClientError as e:
            # If the target doesn't support the policy type, that's expected
            if "PolicyTypeNotEnabledException" in str(e) or "PolicyTypeNotAvailableForOrganizationException" in str(e):
                console.print(f"[dim]Policy type {policy_type.value} not enabled for target {target_id}[/dim]")
            else:
                console.print(f"[yellow]Warning: Could not get {policy_type.value} policies for {target_id}: {str(e)}[/yellow]")
        except Exception as e:
            console.print(f"[yellow]Warning: Unexpected error getting {policy_type.value} policies for {target_id}: {str(e)}[/yellow]")
        
        return policies
    
    def _determine_policy_status(self, policy_data: Dict[str, Any], target_node_type: NodeType) -> str:
        """
