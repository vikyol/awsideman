...
class OperationStore:
    ...
    def get_file_sizes(self) -> Dict[str, int]:
        ...
        return {
            "operations_file": self.operations_file.stat().st_size
            if self.operations_file.exists()
            else 0,
            "rollbacks_file": self.rollbacks_file.stat().st_size
    ...
    def get_storage_stats(self) -> Dict[str, int]:
        ...
        return {
            ...
            "operations_file_size": self.operations_file.stat().st_size
            if self.operations_file.exists()
            else 0,
            "rollbacks_file_size": self.rollbacks_file.stat().st_size
            if self.rollbacks_file.exists()
            else 0,
        }
