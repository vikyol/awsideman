...
class EncryptionError(Exception):
    ...
    def __init__(self, message: str, encryption_type: str = "unknown", original_error: Optional[Exception] = None):
        """
        ...
        if original_error:
            logger.error(f"Encryption error in {encryption_type}: {message} (caused by: {original_error})")
        else:
...
class NoEncryption(EncryptionProvider):
    ...
    def decrypt(self, encrypted_data: bytes) -> Any:
        ...
        try:
            ...
            raise EncryptionError(
                f"Failed to decode bytes to UTF-8: {e}",
                encryption_type="none",
                original_error=e
            )
