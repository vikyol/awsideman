...
class TestBulkOperationsEndToEnd:
    ...
    def mock_aws_client_manager(self):
        ...
        sso_admin_client.delete_account_assignment.return_value = (
            MockAWSResponses.get_successful_assignment_deletion()
        )
    ...
    def test_complete_csv_assign_workflow(self, mock_aws_client_manager, console):
        ...
        try:
            ...
            batch_processor = BatchProcessor(mock_aws_client_manager, batch_size=2)  # noqa: F841

            # Mock the async processing
            with patch("asyncio.run") as mock_asyncio_run:
                # Create mock results
                mock_results = BulkOperationResults(
                    total_processed=2, operation_type="assign", duration=1.5
                )
    ...
    def test_complete_json_revoke_workflow(self, mock_aws_client_manager, console):
        ...
        try:
            ...
            mock_aws_client_manager.get_identity_center_client.return_value.list_account_assignments.return_value = (
                MockAWSResponses.get_existing_assignments()
            )

            batch_processor = BatchProcessor(mock_aws_client_manager, batch_size=3)  # noqa: F841

            # Mock the async processing for revoke
            with patch("asyncio.run") as mock_asyncio_run:
                mock_results = BulkOperationResults(
                    total_processed=3, operation_type="revoke", duration=2.0
                )
    ...
    def test_workflow_with_resolution_errors(self, mock_aws_client_manager, console):
        ...
        try:
            ...
            mock_aws_client_manager.get_identity_store_client.return_value.list_users.return_value = {
                "Users": []
            }
            mock_aws_client_manager.get_identity_center_client.return_value.list_permission_sets.return_value = {
                "PermissionSets": []
            }
            mock_aws_client_manager.get_organizations_client.return_value.list_accounts.return_value = {
                "Accounts": []
            }
            ...
            failed_resolutions = [
                a for a in resolved_assignments if not a.get("resolution_success", True)
            ]
            ...
            batch_processor = BatchProcessor(mock_aws_client_manager, batch_size=1)  # noqa: F841

            with patch("asyncio.run") as mock_asyncio_run:
                mock_results = BulkOperationResults(
                    total_processed=1, operation_type="assign", duration=0.5
                )
...
class TestBulkOperationsPerformance:
    ...
    def test_large_file_processing_performance(self, mock_aws_client_manager):
        ...
        try:
            ...
            )  # Should resolve 1000 records in under 10 seconds with caching
