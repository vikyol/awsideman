...
def test_validate_permission_set_name_empty(mock_console):
    ...
    mock_console.print.assert_called_once_with("[red]Error: Permission set name cannot be empty.[/red]")
...
def test_validate_permission_set_name_too_long(mock_console):
    ...
    mock_console.print.assert_called_once_with("[red]Error: Permission set name cannot exceed 32 characters.[/red]")
...
def test_validate_permission_set_name_invalid_characters(mock_console):
    ...
    for name in invalid_names:
        ...
        mock_console.print.assert_any_call("[red]Error: Permission set name contains invalid characters.[/red]")
        mock_console.print.assert_any_call("[yellow]Permission set names can only contain alphanumeric characters and the following special characters: +=,.@_-[/yellow]")
        mock_console.reset_mock()
...
def test_validate_permission_set_description_too_long(mock_console):
    ...
    mock_console.print.assert_called_once_with("[red]Error: Permission set description cannot exceed 700 characters.[/red]")
...
def test_validate_aws_managed_policy_arn_invalid_format(mock_console):
    ...
    for arn in invalid_arns:
        ...
        mock_console.print.assert_any_call("[yellow]AWS managed policy ARNs should start with 'arn:aws:iam::aws:policy/'.[/yellow]")
        mock_console.print.assert_any_call("[yellow]Example: arn:aws:iam::aws:policy/AdministratorAccess[/yellow]")
        mock_console.reset_mock()
...
def test_format_permission_set_for_display_basic():
    ...
    assert formatted["ARN"] == "arn:aws:sso:::permissionSet/ssoins-1234567890abcdef/ps-1234567890abcdef"
...
def test_resolve_permission_set_identifier_with_name_not_found(mock_console):
    ...
    mock_console.print.assert_any_call(f"[red]Error: Permission set with name '{permission_set_name}' not found.[/red]")
    mock_console.print.assert_any_call("[yellow]Check the permission set name and try again.[/yellow]")
    mock_console.print.assert_any_call("[yellow]Use 'awsideman permission-set list' to see all available permission sets.[/yellow]")
...
@patch("src.awsideman.commands.permission_set.console")
def test_resolve_permission_set_identifier_with_network_error(mock_console, mock_handle_network_error):
    ...
    network_error = EndpointConnectionError(endpoint_url="https://sso-admin.us-east-1.amazonaws.com")
