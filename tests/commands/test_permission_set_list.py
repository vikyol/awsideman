...
def test_list_permission_sets_with_filter(
    ...
    mock_console.print.assert_any_call(
        "[green]Filtered to 1 permission sets matching 'name=Admin'.[/green]"
    )
...
def test_list_permission_sets_with_next_token(
    ...
    # Verify the function called the APIs correctly
    mock_sso_admin.list_permission_sets.assert_called_once_with(
        InstanceArn="arn:aws:sso:::instance/ssoins-1234567890abcdef", NextToken="provided-token"
    )
...
def test_list_permission_sets_with_limit(
    ...
    # Verify the function called the APIs correctly
    mock_sso_admin.list_permission_sets.assert_called_once_with(
        InstanceArn="arn:aws:sso:::instance/ssoins-1234567890abcdef", MaxResults=1
    )
    ...
    # Verify the console output
    mock_console.print.assert_any_call(
        "[green]Found 1 permission sets (showing up to 1 results).[/green]"
    )
