...
class TestCacheStatusCommand:
    ...
    def test_cache_status_with_entries(self, runner, mock_cache_manager):
        ...
        with patch(
            "src.awsideman.commands.cache.CacheManager", return_value=mock_cache_manager
    ...
    def test_cache_status_exception(self, runner):
        ...
        with patch(
            "src.awsideman.commands.cache.CacheManager", side_effect=Exception("Test exception")
        ):
            result = runner.invoke(app, ["status"])
...
class TestCacheWarmCommand:
    ...
    def test_cache_warm_timeout(self, runner, mock_cache_manager):
        ...
        with patch(
            ...
            "src.awsideman.commands.cache.subprocess.run", side_effect=TimeoutExpired("cmd", 300)
        ):
            result = runner.invoke(app, ["warm", "user list"])
...
class TestCacheIntegrationScenarios:
    ...
    def test_cache_warm_multiple_commands(self, runner):
        ...
        commands_to_warm = ["user list", "group list", "permission-set list"]
        ...
        with patch(
            ...
            for i, command in enumerate(commands_to_warm):
                ...
                assert result.exit_code == 0
                assert (
                    f"Added {cache_stats_progression[i + 1]['total_entries'] - cache_stats_progression[i]['total_entries']} new cache entries"
                    in result.stdout
                )
    ...
    def test_cache_warm_with_profile_switching(self, runner):
        ...
        profile_cache_stats = {"profile1": {"total_entries": 0}, "profile2": {"total_entries": 0}}
...
class TestCacheCommandValidation:
    ...
    def test_cache_warm_timeout_handling(self, runner):
        ...
        with patch(
            ...
            "src.awsideman.commands.cache.subprocess.run",
            side_effect=TimeoutExpired("awsideman", 300),
        ):
            result = runner.invoke(app, ["warm", "user list"])
    ...
    def test_cache_warm_subprocess_errors(self, runner):
        ...
        with patch("src.awsideman.commands.cache.CacheManager", return_value=mock_cache_manager):
            ...
            with patch(
                "src.awsideman.commands.cache.subprocess.run", side_effect=FileNotFoundError()
            ):
                result = runner.invoke(app, ["warm", "user list"])
                assert result.exit_code == 1
