...
def sample_user():
    ...
    return {
        ...
        "Emails": [{"Value": "existing.user@example.com", "Primary": True}],
        "Status": "ENABLED",
    }
...
def test_update_user_successful(
    ...
    display_name_op = next(
        (op for op in operations if op.get("AttributePath") == "DisplayName"), None
    )
...
def test_update_user_no_changes(
    ...
    with pytest.raises(typer.Exit):
        update_user(user_id="1234567890")

    # Verify the console output
    mock_console.print.assert_any_call(
        "[yellow]No update parameters provided. User remains unchanged.[/yellow]"
    )
...
def test_update_user_not_found(
    ...
    with pytest.raises(typer.Exit):
        update_user(user_id="nonexistent", display_name="New Display Name")
...
def test_update_user_invalid_email(
    ...
    # Call the function and expect exception
    with pytest.raises(typer.Exit):
        update_user(user_id="1234567890", email="invalid-email")
...
def test_update_user_api_error(
    ...
    # Call the function and expect exception
    with pytest.raises(typer.Exit):
        update_user(user_id="1234567890", display_name="Updated User")
