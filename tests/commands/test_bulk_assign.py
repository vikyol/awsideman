...
class TestBulkAssignCommand:
    ...
    def test_bulk_assign_csv_dry_run(self):
        ...
        try:
            ...
            with patch('awsideman.commands.bulk.validate_profile') as mock_validate_profile, \
                 patch('awsideman.commands.bulk.validate_sso_instance') as mock_validate_sso, \
                 patch('awsideman.commands.bulk.AWSClientManager') as mock_client_manager, \
                 patch('awsideman.commands.bulk.console') as mock_console:
                ...
                with patch('awsideman.utils.bulk.processors.FileFormatDetector.get_processor') as mock_processor, \
                     patch('awsideman.utils.bulk.resolver.ResourceResolver') as mock_resolver, \
                     patch('awsideman.utils.bulk.preview.PreviewGenerator') as mock_preview:
                    ...
                    with pytest.raises(SystemExit) as exc_info:
                        bulk_assign(csv_file, dry_run=True, continue_on_error=True, batch_size=10, profile=None)
        ...
        try:
            ...
            with patch('awsideman.commands.bulk.validate_profile') as mock_validate_profile, \
                ...
                with patch('awsideman.utils.bulk.processors.FileFormatDetector.get_processor') as mock_processor:
                    ...
                    with patch('awsideman.utils.bulk.processors.FileFormatDetector.detect_format') as mock_detect:
                        mock_detect.return_value = 'json'
                        
                        # This will fail at resolution step, but that's expected for this test
                        with pytest.raises(SystemExit):
                            bulk_assign(json_file, dry_run=True, continue_on_error=True, batch_size=10, profile=None)
