...
def test_delete_permission_set_successful_by_name(
    ...
    mock_console.print.assert_any_call("[blue]Deleting permission set 'TestPermissionSet'...[/blue]")
    mock_console.print.assert_any_call("[green]✓ Permission set 'TestPermissionSet' deleted successfully.[/green]")
    mock_console.print.assert_any_call("[green]Permission Set ARN: arn:aws:sso:::permissionSet/ssoins-1234567890abcdef/ps-1234567890abcdef[/green]")
...
def test_delete_permission_set_displays_warning_messages(
    ...
    mock_console.print.assert_any_call("[yellow]Warning: This action cannot be undone. The permission set will be permanently deleted.[/yellow]")
    mock_console.print.assert_any_call("[yellow]If this permission set is assigned to users or groups, those assignments will also be removed.[/yellow]")
...
def test_delete_permission_set_resource_not_found_exception(
    ...
    mock_sso_admin.delete_permission_set.side_effect = ClientError(error_response, "DeletePermissionSet")
    ...
    mock_console.print.assert_any_call("[red]Error: Permission set 'NonExistentPermissionSet' not found.[/red]")
    mock_console.print.assert_any_call("[yellow]The permission set may have been deleted already.[/yellow]")
...
def test_delete_permission_set_conflict_exception(
    ...
    # Verify appropriate error messages are displayed
    mock_console.print.assert_any_call("[red]Error: Cannot delete permission set 'TestPermissionSet'.[/red]")
    mock_console.print.assert_any_call("[yellow]The permission set may be in use by account assignments.[/yellow]")
    mock_console.print.assert_any_call("[yellow]Remove all account assignments for this permission set before deletion.[/yellow]")
...
def test_delete_permission_set_access_denied_exception(
    ...
    mock_console.print.assert_any_call("[yellow]This could be due to insufficient permissions or an issue with the AWS Identity Center service.[/yellow]")
...
def test_delete_permission_set_network_error(
    ...
    mock_sso_admin.describe_permission_set.side_effect = EndpointConnectionError(endpoint_url="https://sso.us-east-1.amazonaws.com")
...
def test_delete_permission_set_unexpected_error(
    ...
    mock_console.print.assert_any_call("[yellow]This is an unexpected error. Please report this issue if it persists.[/yellow]")
