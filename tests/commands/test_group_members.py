...
from src.awsideman.commands.group import _find_user_id, add_member, list_members, remove_member
    ...
    return [
        {
            ...
            "GroupId": "1234567890",
            "MemberId": {"UserId": "0987654321"},
        },
        {
            "MembershipId": "membership-2",
            "GroupId": "1234567890",
            "MemberId": {"UserId": "1122334455"},
        },
    ]
...
def test_list_members_with_pagination(
    ...
    with patch("src.awsideman.commands.group.get_single_key", return_value="q"):
        ...
        # Verify the function called the APIs correctly
        mock_identity_store.list_group_memberships.assert_called_once_with(
            IdentityStoreId="d-1234567890", GroupId=group_id, MaxResults=1
        )
...
def test_list_members_empty_group(
    ...
    mock_console.print.assert_any_call(
        f"[yellow]No members found in group '{sample_group['DisplayName']}'.[/yellow]"
    )
...
def test_list_members_group_not_found(
    ...
    mock_identity_store.list_groups.return_value = {"Groups": [], "NextToken": None}
...
def test_add_member_successful(
    ...
    mock_identity_store.create_group_membership.return_value = {"MembershipId": "new-membership-id"}
    ...
    mock_identity_store.create_group_membership.assert_called_once_with(
        ...
        MemberId={"UserId": sample_user["UserId"]},
    )
    ...
    # Verify the console output
    mock_console.print.assert_any_call(
        f"[green]User '{sample_user['UserName']}' successfully added to group '{sample_group['DisplayName']}'.[/green]"
    )
...
def test_add_member_already_member(
    ...
    # Call the function
    add_member("Administrators", "johndoe")
    ...
    # Verify the console output
    mock_console.print.assert_any_call(
        f"[yellow]User '{sample_user['UserName']}' is already a member of group '{sample_group['DisplayName']}'.[/yellow]"
    )
...
def test_remove_member_successful(
    ...
    mock_identity_store.delete_group_membership.assert_called_once_with(
        IdentityStoreId="d-1234567890", MembershipId="existing-membership-id"
    )
    ...
    # Verify the console output
    mock_console.print.assert_any_call(
        f"[green]User '{sample_user['UserName']}' successfully removed from group '{sample_group['DisplayName']}'.[/green]"
    )
...
def test_remove_member_not_member(
    ...
    # Verify the console output
    mock_console.print.assert_any_call(
        f"[yellow]User '{sample_user['UserName']}' is not a member of group '{sample_group['DisplayName']}'.[/yellow]"
    )
...
def test_find_user_id_by_username(
    ...
    mock_identity_store.list_users.assert_called_once_with(
        IdentityStoreId="d-1234567890",
        Filters=[{"AttributePath": "UserName", "AttributeValue": "johndoe"}],
    )
...
def test_find_user_id_by_email(
    ...
    mock_identity_store.list_users.side_effect = [
        ...
        {"Users": [], "NextToken": None},
        # Second call for listing all users
        {"Users": [sample_user], "NextToken": None},
    ]
...
def test_find_user_id_not_found(
    ...
    # Verify the console output
    mock_console.print.assert_any_call(
        "[red]Error: No user found with identifier 'nonexistent'.[/red]"
    )
