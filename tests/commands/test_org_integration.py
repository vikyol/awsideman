...
from src.awsideman.utils.models import AccountDetails, NodeType, OrgNode, PolicyInfo, PolicyType
...
def mock_organization_tree():
    ...
    engineering_ou = OrgNode(
        id="ou-1234567890-engineering", name="Engineering", type=NodeType.OU, children=[]
    )

    production_ou = OrgNode(
        id="ou-1234567890-production", name="Production", type=NodeType.OU, children=[]
    )

    # Create accounts
    dev_account = OrgNode(id="111111111111", name="dev-account", type=NodeType.ACCOUNT, children=[])

    staging_account = OrgNode(
        id="222222222222", name="staging-account", type=NodeType.ACCOUNT, children=[]
    )

    prod_account = OrgNode(
        id="333333333333", name="prod-account", type=NodeType.ACCOUNT, children=[]
    )
...
def test_complete_org_workflow_table_format(
    ...
    mock_get_account_details.side_effect = lambda client, account_id: mock_account_details[
        account_id
    ]
...
def test_search_command_patterns_and_filters(
    mock_client_manager, mock_org_client, mock_search_accounts, mock_config, mock_account_details
):
    ...
    result = runner.invoke(
        app, ["search", "account", "--ou", "ou-engineering", "--profile", "test-profile"]
    )
    assert result.exit_code == 0
    ...
    result = runner.invoke(
        app, ["search", "account", "--tag", "Environment=Development", "--profile", "test-profile"]
    )
    assert result.exit_code == 0
    mock_search_accounts.assert_called_with(
        organizations_client=mock_org_client.return_value,
...
def test_org_commands_profile_error_scenarios(mock_config):
    ...
    with patch("src.awsideman.commands.org.config") as mock_no_profile:
        mock_no_profile.get.side_effect = lambda key, default=None: {"profiles": {}}.get(
            key, default
        )
...
def test_org_commands_edge_cases(
    ...
    result = runner.invoke(
        app, ["search", "test", "--tag", "URL=https://example.com", "--profile", "test-profile"]
    )
    assert result.exit_code == 0
    mock_search_accounts.assert_called_with(
        organizations_client=mock_org_client.return_value,
...
def test_org_commands_performance_scenarios(
    ...
    for i in range(10):  # 10 roots (unusual but possible)
        ...
        for j in range(5):  # 5 OUs per root
            ...
            for k in range(20):  # 20 accounts per OU
                account = OrgNode(
                    f"{i:03d}{j:03d}{k:06d}", f"account-{i}-{j}-{k}", NodeType.ACCOUNT, []
                )
    ...
    for i in range(50):  # 50 policies
        policy = PolicyInfo(
            ...
            type=PolicyType.SERVICE_CONTROL_POLICY
            if i % 2 == 0
            else PolicyType.RESOURCE_CONTROL_POLICY,
    ...
    result = runner.invoke(
        app, ["trace-policies", "000000000000", "--json", "--profile", "test-profile"]
    )
    assert result.exit_code == 0
