...
def test_create_permission_set_successful_basic(
    ...
    mock_console.print.assert_any_call(
        "[blue]Creating permission set 'TestPermissionSet'...[/blue]"
    )
    mock_console.print.assert_any_call(
        "[green]Permission set 'TestPermissionSet' created successfully.[/green]"
    )
...
def test_create_permission_set_skip_invalid_policy(
    ...
    managed_policies = ["arn:aws:iam::aws:policy/AdministratorAccess", "invalid-policy-arn"]
    result = create_permission_set(
        name="TestPermissionSet",
        description=None,
        session_duration="PT1H",
...
def test_create_permission_set_invalid_policy_arn(
    mock_validate_policy_arn, mock_validate_description, mock_validate_name
):
...
def test_create_permission_set_profile_validation_failure(
    mock_validate_profile, mock_validate_description, mock_validate_name
):
...
def test_create_permission_set_sso_instance_validation_failure(
    mock_validate_sso_instance, mock_validate_profile, mock_validate_description, mock_validate_name
):
...
def test_create_permission_set_duplicate_name_error(
    ...
    mock_sso_admin.create_permission_set.side_effect = ClientError(
        error_response, "CreatePermissionSet"
    )
    ...
    mock_console.print.assert_any_call(
        "[red]Error: Permission set 'ExistingPermissionSet' already exists.[/red]"
    )
    mock_console.print.assert_any_call(
...
def test_create_permission_set_mixed_policy_validation(
    ...
    with pytest.raises(typer.Exit):
        create_permission_set(
            ...
            session_duration="PT1H",
            relay_state=None,
            managed_policy=["arn:aws:iam::aws:policy/AdministratorAccess", "invalid-arn"],
            profile=None,
        )
