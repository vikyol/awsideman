...
from src.awsideman.aws_clients.manager import OrganizationsClient, PolicyResolver
from src.awsideman.utils.models import HierarchyPath, NodeType, PolicyInfo, PolicyType
...
class TestPolicyResolver:
    ...
    def test_resolve_policies_for_account_with_policy_errors(
        self, policy_resolver, mock_organizations_client
    ):
    ...
    def test_get_hierarchy_path_success(self, policy_resolver, mock_organizations_client):
        ...
        mock_organizations_client.list_parents.side_effect = [
            ...
            [
                {"Id": "r-1234", "Type": "ROOT"}
            ],  # OU's parent (called again to get grandparent for OU name lookup)
        ]

        # Mock root details
        mock_organizations_client.list_roots.return_value = [{"Id": "r-1234", "Name": "Root"}]
    ...
    def test_get_hierarchy_path_account_directly_under_root(
        self, policy_resolver, mock_organizations_client
    ):
    ...
    def test_get_hierarchy_path_with_errors(self, policy_resolver, mock_organizations_client):
        ...
        mock_organizations_client.list_parents.side_effect = [[{"Id": "r-1234", "Type": "ROOT"}]]
    ...
    def test_get_policies_for_target_scp_success(self, policy_resolver, mock_organizations_client):
        ...
        with patch.object(policy_resolver, "_determine_policy_status", return_value="ENABLED"):
            result = policy_resolver._get_policies_for_target(
                target_id, PolicyType.SERVICE_CONTROL_POLICY, target_name, NodeType.OU
            )
    ...
    def test_get_policies_for_target_rcp_success(self, policy_resolver, mock_organizations_client):
        ...
        with patch.object(policy_resolver, "_determine_policy_status", return_value="CONDITIONAL"):
            result = policy_resolver._get_policies_for_target(
                target_id, PolicyType.RESOURCE_CONTROL_POLICY, target_name, NodeType.ROOT
            )
    ...
    def test_get_policies_for_target_policy_type_not_enabled(
        self, policy_resolver, mock_organizations_client
    ):
        ...
        client_error = ClientError(
            error_response={
                "Error": {
                    ...
                    "Message": "Policy type not enabled",
                }
            },
            operation_name="ListPoliciesForTarget",
        )
        ...
        result = policy_resolver._get_policies_for_target(
            target_id, PolicyType.RESOURCE_CONTROL_POLICY, "Engineering", NodeType.OU
        )

        # Should return empty list, not raise exception
        assert result == []

    def test_get_policies_for_target_other_client_error(
        self, policy_resolver, mock_organizations_client
    ):
        ...
        result = policy_resolver._get_policies_for_target(
            target_id, PolicyType.SERVICE_CONTROL_POLICY, "Engineering", NodeType.OU
        )

        # Should return empty list and log warning
        assert result == []

    def test_get_policies_for_target_malformed_policy_data(
        self, policy_resolver, mock_organizations_client
    ):
    ...
    def test_determine_policy_status_enabled_default(self, policy_resolver):
        ...
        policy_data = {"Id": "p-1234", "Name": "TestPolicy"}
    ...
    def test_determine_policy_status_conditional(self, policy_resolver):
        ...
        policy_data = {"Id": "p-1234", "Name": "TestPolicy", "Type": "CONDITIONAL_POLICY"}
    ...
    def test_determine_policy_status_disabled(self, policy_resolver):
        ...
        policy_data = {"Id": "p-1234", "Name": "TestPolicy", "Status": "DISABLED"}
    ...
    def test_scp_vs_rcp_distinction(self, policy_resolver, mock_organizations_client):
        ...
        hierarchy_path = HierarchyPath(ids=["r-1234"], names=["Root"], types=[NodeType.ROOT])
        ...
        with patch.object(policy_resolver, "_get_hierarchy_path", return_value=hierarchy_path):
            with patch.object(policy_resolver, "_get_policies_for_target") as mock_get_policies:
                ...
                mock_get_policies.side_effect = [[scp_policy], [rcp_policy]]  # SCP call  # RCP call
