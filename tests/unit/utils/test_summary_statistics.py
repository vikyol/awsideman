...
class TestSummaryStatisticsCollector:
    ...
    async def test_summary_statistics_model_methods(self):
        ...
        stats = SummaryStatistics(
            ...
            last_updated=datetime.now(timezone.utc),
            user_creation_dates={
                "user-1": datetime(2023, 1, 1, tzinfo=timezone.utc),
                "user-2": datetime(2023, 6, 15, tzinfo=timezone.utc),
            },
        )
        ...
        assert newest_date == datetime(2023, 6, 15, tzinfo=timezone.utc)
        assert oldest_date == datetime(2023, 1, 1, tzinfo=timezone.utc)
