...
class TestCacheManagerConfigurationInit:
    ...
    def test_init_with_provided_advanced_config(self):
        ...
        config = AdvancedCacheConfig(
            enabled=True, backend_type="file", encryption_enabled=False, default_ttl=2400
        )

        with tempfile.TemporaryDirectory() as temp_dir:
            with (
                patch("src.awsideman.cache.manager.BackendFactory") as mock_factory,
    ...
    def test_init_with_invalid_advanced_config_falls_back(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            with patch(
                "src.awsideman.cache.manager.CacheManager._load_basic_cache_config"
            ) as mock_load_basic:
    ...
    def test_init_loads_from_environment(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            with (
                ...
                # The CacheManager might fall back to basic CacheConfig if advanced config fails
                assert isinstance(manager.config, (CacheConfig, AdvancedCacheConfig))

                if isinstance(manager.config, AdvancedCacheConfig):
    ...
    def test_init_backend_creation_failure_with_fallback(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            with (
                ...
                mock_factory.validate_backend_availability.return_value = True
                mock_factory.create_backend_with_fallback.side_effect = CacheBackendError(
                ...
                with (
                    patch(
                        ...
                        return_value=mock_file_backend,
                    ),
    ...
    def test_init_complete_failure_disables_cache(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            with patch("src.awsideman.cache.manager.BackendFactory") as mock_factory:
                ...
                with patch(
                    "src.awsideman.cache.backends.file.FileBackend",
                    side_effect=Exception("File backend failed"),
                ):
                    manager = CacheManager(config=config, base_cache_dir=temp_dir)
    ...
    def test_init_encryption_failure_falls_back_to_no_encryption(self):
        ...
        config = AdvancedCacheConfig(
            enabled=True, backend_type="file", encryption_enabled=True, encryption_type="aes256"
        )
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            with patch(
                ...
                with (
                    patch("src.awsideman.cache.manager.BackendFactory") as mock_factory,
                    patch(
                        "src.awsideman.cache.manager.EncryptionProviderFactory"
                    ) as mock_enc_factory,
                ):
                    mock_backend = Mock()
                    mock_factory.validate_backend_availability.return_value = True
