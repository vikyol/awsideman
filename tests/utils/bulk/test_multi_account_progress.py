...
class TestMultiAccountProgressTracker:
    ...
    def test_start_multi_account_progress(self, tracker):
        ...
        with patch.object(tracker, 'start_progress') as mock_start:
            ...
            mock_start.assert_called_once_with(10, "Processing assign operations across 10 accounts")
    ...
    def test_record_account_result_with_live_display(self, tracker):
        ...
        with patch.object(tracker, 'update_progress'), \
    ...
    def test_display_final_summary(self, tracker):
        ...
        failed_accounts = [
            AccountResult("123456789014", "Account3", "failed", "Error message")
        ]
        skipped_accounts = []
        ...
        with patch.object(tracker, 'finish_progress') as mock_finish, \
    ...
    def test_display_final_summary_complete_success(self, tracker):
        ...
        with patch.object(tracker, 'finish_progress'), \
            ...
            success_messages = [msg for msg in print_calls if "All 2 accounts processed successfully" in str(msg)]
    ...
    @patch('src.awsideman.utils.bulk.multi_account_progress.Live')
    @patch('src.awsideman.utils.bulk.multi_account_progress.Table')
    ...
    def test_display_failed_accounts_with_failures(self, tracker):
        ...
        failed_accounts = [
            AccountResult("123456789012", "Account1", "failed", "Error 1", processing_time=1.0, retry_count=2),
            AccountResult("123456789013", "Account2", "failed", "Error 2", processing_time=2.0, retry_count=1)
        ]
    ...
    def test_inheritance_from_progress_tracker(self, tracker):
        ...
        from src.awsideman.utils.bulk.batch import ProgressTracker
