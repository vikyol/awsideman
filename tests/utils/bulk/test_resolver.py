...
class TestResourceResolver:
    ...
    def test_resolve_assignment_success(self, resource_resolver):
        ...
        resource_resolver.resolve_principal_name = Mock(return_value=ResolutionResult(
            success=True, resolved_value='user-1234567890abcdef'
        ))
        resource_resolver.resolve_permission_set_name = Mock(return_value=ResolutionResult(
            success=True, resolved_value='arn:aws:sso:::permissionSet/ssoins-1234567890abcdef/ps-1234567890abcdef'
        ))
        resource_resolver.resolve_account_name = Mock(return_value=ResolutionResult(
            success=True, resolved_value='123456789012'
        ))
        ...
        assert result['permission_set_arn'] == 'arn:aws:sso:::permissionSet/ssoins-1234567890abcdef/ps-1234567890abcdef'
...
class TestAssignmentValidator:
    ...
    def test_validate_assignment_success(self, assignment_validator):
        ...
        assert len(errors) == 0
        assignment_validator.validate_principal.assert_called_once_with('user-1234567890abcdef', 'USER')
