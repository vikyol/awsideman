...
def test_account_matches_ou_filter(sample_accounts):
    ...
    # Test exact OU match
    assert _account_matches_ou_filter(dev_account, "Engineering") == True
    assert _account_matches_ou_filter(dev_account, "Development") == True
    assert _account_matches_ou_filter(dev_account, "Root") == True
    assert _account_matches_ou_filter(dev_account, "QA") == False
    
    assert _account_matches_ou_filter(qa_account, "QA") == True
    assert _account_matches_ou_filter(qa_account, "Engineering") == False


def test_account_matches_tag_filter(sample_accounts):
    """Test the tag filter matching function."""
    dev_account = sample_accounts[0]  # Has tags: {"Environment": "Development", "Team": "Engineering"}
    qa_account = sample_accounts[2]   # Has tags: {"Environment": "Testing", "Team": "QA"}
    
    # Test single tag match
    assert _account_matches_tag_filter(dev_account, {"Environment": "Development"}) == True
    assert _account_matches_tag_filter(dev_account, {"Team": "Engineering"}) == True
    assert _account_matches_tag_filter(dev_account, {"Environment": "Production"}) == False
    assert _account_matches_tag_filter(dev_account, {"Team": "QA"}) == False
    
    # Test multiple tag match (all must match)
    assert _account_matches_tag_filter(dev_account, {"Environment": "Development", "Team": "Engineering"}) == True
    assert _account_matches_tag_filter(dev_account, {"Environment": "Development", "Team": "QA"}) == False
    
    # Test non-existent tag
    assert _account_matches_tag_filter(dev_account, {"NonExistent": "Value"}) == False