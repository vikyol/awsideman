...
class TestCachedAwsClient:
    ...
    def test_generate_cache_key(self):
        ...
        key3 = self.cached_client._generate_cache_key('describe_account', {'account_id': '123456789012'})
        assert key1 != key3
        
        # Different parameters should have different keys
        key4 = self.cached_client._generate_cache_key('describe_account', {'account_id': '123456789013'})
        assert key3 != key4
    ...
    def test_is_cacheable_operation(self):
        ...
        for op in cacheable_ops:
            assert self.cached_client._is_cacheable_operation(op), f"Operation {op} should be cacheable"
        ...
        for op in non_cacheable_ops:
            assert not self.cached_client._is_cacheable_operation(op), f"Operation {op} should not be cacheable"
    ...
    def test_execute_with_cache_key_generation_error(self):
        ...
        with patch.object(self.cached_client, '_generate_cache_key', side_effect=Exception("Key generation error")):
...
class TestCreateCachedClientManager:
    ...
    def test_create_cached_client_manager(self, mock_cache_manager_class, mock_client_manager_class):
        ...
        mock_client_manager_class.assert_called_once_with(profile='test-profile', region='us-west-2')
...
class TestCacheableOperations:
    ...
    def test_all_read_operations_are_cacheable(self):
        ...
        for operation in expected_cacheable:
            assert self.cached_client._is_cacheable_operation(operation), f"{operation} should be cacheable"
    ...
    def test_write_operations_are_not_cacheable(self):
        ...
        for operation in write_operations:
            assert not self.cached_client._is_cacheable_operation(operation), f"{operation} should not be cacheable"
...
class TestCacheIntegrationScenarios:
    ...
    def test_cache_hit_miss_sequence(self):
        ...
        result1 = self.cached_client._execute_with_cache('list_accounts_for_parent', {'parent_id': 'ou-123'}, api_call)
        ...
        result2 = self.cached_client._execute_with_cache('list_accounts_for_parent', {'parent_id': 'ou-123'}, api_call)
    ...
    def test_different_parameters_different_cache_keys(self):
        ...
        self.cached_client._execute_with_cache('describe_account', {'account_id': '111111111111'}, api_call1)
        
        # Call with second set of parameters
        api_call2 = Mock(return_value={'result': 'second'})
        self.cached_client._execute_with_cache('describe_account', {'account_id': '222222222222'}, api_call2)
