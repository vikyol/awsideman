...
class TestEncryptionInfrastructureIntegration:
    ...
    def test_complete_encryption_workflow(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            with patch.object(key_manager, "is_keyring_available", return_value=False):
                ...
                provider = EncryptionProviderFactory.create_provider(
    ...
    def test_key_rotation_with_encryption(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            with patch.object(key_manager, "is_keyring_available", return_value=False):
                ...
                new_provider = EncryptionProviderFactory.create_provider(
                    "aes256", key_manager=key_manager
                )
    ...
    def test_factory_provider_availability(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            key_manager = FallbackKeyManager(fallback_dir=temp_dir)
            with patch.object(key_manager, "is_keyring_available", return_value=False):
                aes_provider = EncryptionProviderFactory.create_provider(
                    "aes256", key_manager=key_manager
                )
    ...
    def test_configuration_validation(self):
        ...
        assert (
            EncryptionProviderFactory.validate_provider_config("aes256", {"key_manager": "config"})
            is True
        )
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            with patch.object(key_manager, "is_keyring_available", return_value=False):
                ...
                for size_name, data in test_data.items():
                    ...
                    assert (
                        encrypt_time < 1.0
                    ), f"Encryption too slow for {size_name} data: {encrypt_time}s"
                    assert (
                        decrypt_time < 1.0
                    ), f"Decryption too slow for {size_name} data: {decrypt_time}s"
