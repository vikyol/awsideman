...
from src.awsideman.cache.backends.dynamodb import DynamoDBBackend, MAX_ITEM_SIZE, CHUNK_SIZE, COMPRESSION_THRESHOLD
...
class TestDynamoDBChunking:
    ...
    def test_cleanup_chunks(self, mock_table):
        ...
        mock_table.scan.return_value = {
            'Items': [
                {'cache_key': 'key#chunk#id#0'},
                {'cache_key': 'key#chunk#id#1'}
            ]
        }
    ...
    def test_cleanup_orphaned_chunks(self, mock_table):
        ...
        mock_table.scan.return_value = {
            'Items': [
                {
                    'cache_key': 'key1#chunk#id1#0',
                    'parent_key': 'key1',
                    'chunk_id': 'id1'
                },
                {
                    'cache_key': 'key2#chunk#id2#0',
                    'parent_key': 'key2',
                    'chunk_id': 'id2'
                }
            ]
