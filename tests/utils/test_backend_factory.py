...
class TestBackendFactory:
    ...
    def test_create_dynamodb_backend(self):
        ...
        with patch('src.awsideman.cache.backends.dynamodb.DynamoDBBackend', return_value=mock_dynamodb_backend):
            backend = BackendFactory.create_backend(config)
    ...
    def test_create_dynamodb_backend_import_error(self):
        ...
        with patch('src.awsideman.cache.backends.dynamodb.DynamoDBBackend', side_effect=ImportError("boto3 not found")):
    ...
    def test_create_hybrid_backend(self):
        ...
        with patch('src.awsideman.cache.backends.hybrid.HybridBackend', return_value=mock_hybrid_backend):
            with patch.object(BackendFactory, '_create_file_backend', return_value=mock_file_backend):
                with patch.object(BackendFactory, '_create_dynamodb_backend', return_value=mock_dynamodb_backend):
                    backend = BackendFactory.create_backend(config)
    ...
    def test_create_hybrid_backend_import_error(self):
        ...
        with patch('src.awsideman.cache.backends.hybrid.HybridBackend', side_effect=ImportError("Missing dependency")):
            with pytest.raises(CacheBackendError) as exc_info:
                BackendFactory.create_backend(config)
    ...
    def test_create_backend_unexpected_error(self):
        ...
        with patch.object(BackendFactory, '_create_file_backend', side_effect=Exception("Unexpected error")):
            with pytest.raises(CacheBackendError) as exc_info:
                BackendFactory.create_backend(config)
    ...
    def test_create_backend_with_fallback_to_file(self):
        ...
        with patch.object(BackendFactory, 'create_backend', side_effect=CacheBackendError("DynamoDB failed")):
    ...
    def test_create_backend_with_fallback_file_primary_fails(self):
        ...
        with patch.object(BackendFactory, 'create_backend', side_effect=CacheBackendError("File backend failed")):
            with pytest.raises(CacheBackendError) as exc_info:
    ...
    def test_create_backend_with_fallback_both_fail(self):
        ...
        with patch.object(BackendFactory, 'create_backend', side_effect=CacheBackendError("DynamoDB failed")):
            with patch.object(BackendFactory, '_create_file_backend', side_effect=Exception("File fallback failed")):
                with pytest.raises(CacheBackendError) as exc_info:
                    BackendFactory.create_backend_with_fallback(config)
                
                assert "Both dynamodb backend and file backend fallback failed" in str(exc_info.value)
    ...
    def test_create_file_backend_error(self):
        ...
        with patch('src.awsideman.cache.backends.file.FileBackend', side_effect=Exception("Creation failed")):
            with pytest.raises(CacheBackendError) as exc_info:
    ...
    def test_create_dynamodb_backend_success(self):
        ...
        with patch('src.awsideman.cache.backends.dynamodb.DynamoDBBackend', return_value=mock_backend) as mock_class:
    ...
    def test_create_dynamodb_backend_error(self):
        ...
        with patch('src.awsideman.cache.backends.dynamodb.DynamoDBBackend', side_effect=Exception("Creation failed")):
            with pytest.raises(CacheBackendError) as exc_info:
    ...
    def test_create_hybrid_backend_success(self):
        ...
        with patch('src.awsideman.cache.backends.hybrid.HybridBackend', return_value=mock_hybrid_backend) as mock_class:
            with patch.object(BackendFactory, '_create_file_backend', return_value=mock_file_backend):
                with patch.object(BackendFactory, '_create_dynamodb_backend', return_value=mock_dynamodb_backend):
    ...
    def test_create_hybrid_backend_sub_backend_error(self):
        ...
        with patch.object(BackendFactory, '_create_file_backend', side_effect=CacheBackendError("File failed")):
            with pytest.raises(CacheBackendError) as exc_info:
    ...
    def test_create_hybrid_backend_error(self):
        ...
        with patch('src.awsideman.cache.backends.hybrid.HybridBackend', side_effect=Exception("Creation failed")):
            with patch.object(BackendFactory, '_create_file_backend', return_value=mock_file_backend):
                with patch.object(BackendFactory, '_create_dynamodb_backend', return_value=mock_dynamodb_backend):
                    with pytest.raises(CacheBackendError) as exc_info:
                        BackendFactory._create_hybrid_backend(config)
