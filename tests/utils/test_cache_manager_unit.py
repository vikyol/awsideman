...
class TestCacheManagerSizeManagement(unittest.TestCase):
    ...
    def test_check_and_manage_cache_size_over_limit(self):
        ...
        with patch.object(self.cache_manager.path_manager, 'get_cache_size', return_value=2 * 1024 * 1024):  # 2MB
            with patch.object(self.cache_manager, '_cleanup_by_size', return_value=5) as mock_cleanup:
                self.cache_manager._check_and_manage_cache_size()
                mock_cleanup.assert_called_once()
    ...
    def test_cleanup_by_size(self):
        ...
        with patch.object(self.cache_manager.path_manager, 'list_cache_files', return_value=test_files):
    ...
    def test_get_cache_size_info(self):
        ...
        required_keys = [
            'current_size_bytes', 'current_size_mb', 'max_size_bytes', 'max_size_mb',
            'usage_percentage', 'is_over_limit', 'bytes_over_limit',
            'available_space_bytes', 'available_space_mb'
        ]
    ...
    def test_get_cache_size_info_error(self):
        ...
        with patch.object(self.cache_manager.path_manager, 'get_cache_size', side_effect=Exception("Size error")):
            size_info = self.cache_manager.get_cache_size_info()
...
class TestCacheManagerStats(unittest.TestCase):
    ...
    def test_get_cache_stats_error(self):
        ...
        with patch.object(self.cache_manager.path_manager, 'list_cache_files', side_effect=Exception("Stats error")):
            stats = self.cache_manager.get_cache_stats()
