...
class TestDynamoDBIntegration:
    ...
    def test_dynamodb_cache_operations_integration(self, mock_session):
        ...
        config = AdvancedCacheConfig(
            backend_type="dynamodb", dynamodb_table_name="cache-ops-table", encryption_enabled=False
        )
    ...
    def test_dynamodb_health_check_integration(self, mock_session):
        ...
        config = AdvancedCacheConfig(
            backend_type="dynamodb", dynamodb_table_name="health-check-table"
        )
    ...
    def test_dynamodb_stats_integration(self, mock_session):
        ...
        config = AdvancedCacheConfig(
            backend_type="dynamodb", dynamodb_table_name="stats-table", dynamodb_region="eu-west-1"
        )
    ...
    def test_dynamodb_with_encryption_integration(self, mock_session):
        ...
        sensitive_data = {
            ...
            "personal_info": {"email": "user@example.com", "phone": "+1-555-0123"},
        }
    ...
    def test_dynamodb_backend_unavailable_fallback(self):
        ...
        with patch(
            "src.awsideman.cache.backends.dynamodb.boto3",
            side_effect=ImportError("boto3 not available"),
        ):
    ...
    def test_dynamodb_error_handling_integration(self, mock_session):
        ...
        config = AdvancedCacheConfig(
            backend_type="dynamodb", dynamodb_table_name="error-test-table"
        )
        ...
        mock_client.describe_table.side_effect = ClientError(
            {"Error": {"Code": "AccessDenied", "Message": "Access denied"}}, "DescribeTable"
        )
    ...
    def test_dynamodb_table_validation_integration(self, mock_session):
        ...
        config = AdvancedCacheConfig(
            backend_type="dynamodb", dynamodb_table_name="validation-table"
        )
        ...
        assert len(validation_result["errors"]) > 0
        assert any(
            "Key schema should have 'cache_key' as HASH key" in error
    ...
    def test_dynamodb_large_data_chunking_integration(self, mock_session):
        ...
        config = AdvancedCacheConfig(backend_type="dynamodb", dynamodb_table_name="chunking-table")
