...
from src.awsideman.utils.security import (
    SecureMemory, TimingProtection, InputValidator, SecureLogger,
    secure_memory, timing_protection, input_validator, get_secure_logger
)
...
class TestInputValidator:
    ...
    def test_sanitize_cache_key(self):
        ...
        for input_key, expected in test_cases:
            ...
            assert result == expected, f"Sanitization failed for {input_key}: got {result}, expected {expected}"
    ...
    def test_validate_aws_account_id(self):
        ...
        for account_id in valid_ids:
            assert input_validator.validate_aws_account_id(account_id), f"Account ID should be valid: {account_id}"
        
        for account_id in invalid_ids:
            assert not input_validator.validate_aws_account_id(account_id), f"Account ID should be invalid: {account_id}"
    ...
    def test_validate_uuid(self):
        ...
        for uuid_str in invalid_uuids:
            assert not input_validator.validate_uuid(uuid_str), f"UUID should be invalid: {uuid_str}"
    ...
    def test_validate_file_path(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            for path in invalid_paths:
                assert not input_validator.validate_file_path(path, allowed_dirs), f"Path should be invalid: {path}"
