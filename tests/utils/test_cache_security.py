...
from src.awsideman.encryption.provider import EncryptionProviderFactory, EncryptionError
from src.awsideman.encryption.key_manager import FallbackKeyManager, KeyManager
...
class TestCacheSecurity:
    ...
    def test_cache_file_permissions(self):
        ...
        if os.name == 'posix':
            expected_mode = 0o600
            assert file_mode == expected_mode, f"Cache file permissions too permissive: {oct(file_mode)}"
    ...
    def test_error_information_leakage(self):
        ...
        try:
            ...
            assert "Invalid encrypted data" in error_message or "AES decryption failed" in error_message
