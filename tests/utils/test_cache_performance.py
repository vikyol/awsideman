...
class TestCachePerformance:
    ...
    def test_encryption_decryption_performance(self):
        ...
        for size_name, test_data in test_cases:
            ...
            for _ in range(100):  # 100 iterations
                ...
                no_encryption.decrypt(encrypted)
            ...
            if size_name != "large":  # Large data may have higher overhead
                assert (
                    overhead_ms < 10
                ), f"Encryption overhead too high for {size_name}: {overhead_ms}ms"

        # Print results for analysis
        print("\nEncryption Performance Results:")
        for size_name, result in results.items():
            print(
                f"{size_name.capitalize()} data ({result['data_size']} bytes): "
    ...
    def test_dynamodb_backend_performance(self, mock_session):
        ...
        config = AdvancedCacheConfig(backend_type="dynamodb", dynamodb_table_name="perf-test-table")
    ...
    def test_concurrent_access_performance(self):
        ...
        print(
            f"Success rate: {success_count}/{total_operations} ({success_count/total_operations*100:.1f}%)"
        )
        ...
        assert (
            total_operations / total_time > 50
        ), f"Throughput too low: {total_operations/total_time:.1f} ops/sec"
