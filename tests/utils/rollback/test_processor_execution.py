...
class TestRollbackProcessorExecution:
    ...
    def test_execute_rollback_revoke_success(self):
        ...
        # Act
        with patch(
            "src.awsideman.utils.rollback.processor.uuid.uuid4", return_value="rollback-123"
        ):
    ...
    def test_execute_rollback_assign_success(self):
        ...
        # Act
        with patch(
            "src.awsideman.utils.rollback.processor.uuid.uuid4", return_value="rollback-456"
        ):
            result = self.processor.execute_rollback(plan, dry_run=False)
    ...
    def test_execute_rollback_access_denied_error(self):
        ...
        assert "Account 123456789012: AccessDenied - Insufficient permissions" in result.errors[0]
    ...
    def test_execute_rollback_mixed_success_failure(self):
        ...
        assert result.failed_actions == 1
        assert len(result.errors) == 1
        assert "Account 123456789013: ThrottlingException - Rate exceeded" in result.errors[0]
    ...
    def test_execute_rollback_unexpected_error(self):
        ...
        assert result.failed_actions == 1
        assert len(result.errors) == 1
        assert "Account 123456789012: Unexpected error - Unexpected error" in result.errors[0]
    ...
    def test_log_rollback_operation(self):
        ...
        from src.awsideman.utils.rollback.models import OperationRecord, OperationType
