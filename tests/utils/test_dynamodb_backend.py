...
class TestDynamoDBBackendTableManagement:
    ...
    @patch.object(DynamoDBBackend, 'client', new_callable=lambda: Mock())
    @patch.object(DynamoDBBackend, '_is_ttl_enabled')
    def test_validate_table_schema_invalid_key_schema(self, mock_is_ttl_enabled, mock_client, mock_check_table):
        ...
        assert result['valid'] is False
        assert any("Key schema should have 'cache_key' as HASH key" in error for error in result['errors'])
        ...
        mock_validate.side_effect = [
            {
                'valid': False,
                'errors': [],
                'warnings': ['TTL is not enabled on the table']
            },
            {
                'valid': True,
                'errors': [],
