...
class ImportValidator:
                    ...
                    if node.module:
                        ...
                        for alias in node.names:
                            if alias.name == "*":
                                ...
                                imports.append(
                                    (f"{node.module}.{alias.name}", node.lineno, "from_import")
                                )
                    else:
                        ...
                        if level > 0:
                            ...
                            for alias in node.names:
                                imports.append(
                                    (f"{'.' * level}{alias.name}", node.lineno, "relative")
                                )
    ...
    def validate_import(
        self, import_name: str, file_path: Path, line_no: int, import_type: str
    ) -> bool:
        ...
        for pattern in skip_patterns:
            if import_name.startswith(pattern):
                self.skipped_imports.append(
                    (import_name, str(file_path), line_no, "external_dependency")
                )
                return True

        # For internal imports, use path-based validation instead of actual import
        if (
            import_name.startswith("awsideman")
            or import_name.startswith("src.awsideman")
            or import_name.startswith(".")
            or import_type == "relative"
        ):
        ...
        try:
            ...
            self.broken_imports.append(
                (import_name, str(file_path), line_no, f"Unexpected error: {str(e)}")
            )
            return False

    def validate_internal_import_path(
        self, import_name: str, file_path: Path, line_no: int, import_type: str
    ) -> bool:
        ...
        if package_path.exists():
                ...
                if not self.check_attribute_in_package(package_path, attr_name):
                    self.broken_imports.append(
                        (
                            ...
                            str(file_path),
                            line_no,
                            f"Attribute '{attr_name}' not found in package",
                        )
                    )
                    return False
            self.successful_imports.append((import_name, str(file_path), line_no))
            ...
            self.broken_imports.append(
                (
                    ...
                    str(file_path),
                    line_no,
                    f"Module/package not found at {expected_path}",
                )
            )
            return False

    def validate_relative_import_path(
        self, import_name: str, file_path: Path, line_no: int
    ) -> bool:
            ...
            if target_path.exists() or target_package.exists():
                ...
                self.broken_imports.append(
                    (
                        ...
                        str(file_path),
                        line_no,
                        f"Relative import target not found: {target_path}",
                    )
                )
                return False
        else:
            ...
            if init_file.exists():
                ...
                self.broken_imports.append(
                    (
                        ...
                        str(file_path),
                        line_no,
                        f"Package __init__.py not found: {init_file}",
                    )
                )
                return False
    ...
    def check_attribute_in_module(self, module_path: Path, attr_name: str) -> bool:
        ...
        try:
            ...
            return (
                f"class {attr_name}" in content
                or f"def {attr_name}" in content
    ...
    def print_report(self):
        ...
        stats = {
            "total": len(self.successful_imports)
            + len(self.broken_imports)
            + len(self.skipped_imports),
            "successful": len(self.successful_imports),
            "broken": len(self.broken_imports),
            "skipped": len(self.skipped_imports),
        }
